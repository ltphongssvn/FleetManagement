...
export const GATE_COVERAGE_EXIT = {
  ...
  coverage: 1,
  merge: 3,
  // ONE code per outcome. An absent container runtime and an unspawnable
  // binary are both HOST problems, and a caller branches on the NUMBER, so
  // two names for 4 would be indistinguishable downstream -- which is why
  // the distinct-codes guard refuses them. The specific diagnosis belongs
  // in the message (containerRuntimeUnavailableMessage), not in a second
  // constant sharing a value.
  host: 4,
} as const;
