...
describe('recipientsInCiphertext', () => {
  ...
  it('reads NOTHING from the encrypted values themselves', () => {
    expect(recipientsInCiphertext('FOO: ENC[AES256_GCM,data:age1notakey,type:str]'))
      .toEqual([]);
  });
...
describe('recipientDrift: agreement', () => {
  it('reports NO drift when both sides carry the same keys', () => {
    expect(recipientDrift(roster([A, B]), ciphertextFor([A, B])))
      .toEqual({ lockedOut: [], staleGrants: [] });
  });
  ...
  it('ignores ORDER, which differs between the two files by construction', () => {
    expect(recipientDrift(roster([A, B]), ciphertextFor([B, A])))
      .toEqual({ lockedOut: [], staleGrants: [] });
  });
});
...
describe('recipientDrift: the lockout', () => {
  ...
  it('names a machine granted in the roster that CANNOT decrypt', () => {
    expect(recipientDrift(roster([A, B, C]), ciphertextFor([A, B])))
      .toEqual({ lockedOut: [C], staleGrants: [] });
  });

  it('names the remedy: re-encrypt from a machine holding the plaintext', () => {
    const message = describeRecipientDrift(
      recipientDrift(roster([A, C]), ciphertextFor([A])),
    );
...
describe('recipientDrift: the stale grant', () => {
  ...
  it('names a key that can decrypt but is no longer in the roster', () => {
    expect(recipientDrift(roster([A]), ciphertextFor([A, B])))
      .toEqual({ lockedOut: [], staleGrants: [B] });
  });
  ...
  it('names ROTATION as part of the remedy, not just re-encryption', () => {
    const message = describeRecipientDrift(
      recipientDrift(roster([A]), ciphertextFor([A, B])),
    );
  ...
  it('reports BOTH directions at once when both are present', () => {
    expect(recipientDrift(roster([A, C]), ciphertextFor([A, B])))
      .toEqual({ lockedOut: [C], staleGrants: [B] });
  });

  it('says so plainly when there is nothing to report', () => {
    expect(describeRecipientDrift({ lockedOut: [], staleGrants: [] }))
      .toContain('agree');
  });
});
