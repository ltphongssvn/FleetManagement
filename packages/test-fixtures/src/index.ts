...
export { createPgUniqueViolation, createWrappedError } from './pg-error-fixtures.js';
...
export { testSha, testShortSha, INVALID_SHA_FIXTURES } from './provenance-fixtures.js';
...
export { testActionId, testAggregateId } from './id-fixtures.js';
...
export { createListAssignedRow, createListAssignedStop } from './list-assigned-fixtures.js';

// JSONC reading for guards that assert against turbo.jsonc / tsconfig.json.
// Lives here, not in scripts/, because FIVE separate guards each carried their
// own copy -- four under scripts/ and one in apps/api/test/ -- and a relative
// import across the package boundary (../../../scripts/...) is the documented
// anti-pattern: it bypasses workspace resolution and breaks on any file move.
// apps consume packages, so the shared helper belongs in the shared package and
// is imported by NAME.
export { parseJsonc, readJsonc, readTurboTasks, type TurboTask } from './jsonc-fixtures.js';
