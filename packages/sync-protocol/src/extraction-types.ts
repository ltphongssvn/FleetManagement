...
const ExtractionMimeSchema = z.enum(
...
export const ExtractionJobDataWireSchema = z
  .object({
    manifestId: z.guid(),
...
export const ExtractionResultWireSchema = z
  .object({
    manifestId: z.guid(),
  ...
  .strict()
  .refine((v) => (v.status === 'extracted') === (v.extractedNetWeightKg !== null), {
    message: 'extractedNetWeightKg must be non-null iff status is extracted',
  })
  .refine((v) => (v.status !== 'extracted') === (v.reason !== undefined), {
