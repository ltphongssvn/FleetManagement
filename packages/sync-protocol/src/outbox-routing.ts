...
export type OutboxRoutingDecision =
  | { readonly accepted: true; readonly queueName: OutboxTargetQueue; readonly policyVersion: typeof OUTBOX_ROUTING_POLICY_VERSION }
  | { readonly accepted: false; readonly rejectionCode: OutboxRoutingRejectionCode; readonly policyVersion: typeof OUTBOX_ROUTING_POLICY_VERSION };
...
export function routeOutboxRow(input: OutboxRoutingInput): OutboxRoutingDecision {
  ...
  return { accepted: false, rejectionCode: 'unknown_aggregate', policyVersion: OUTBOX_ROUTING_POLICY_VERSION };
}
