...
describe('ErpInvoicePayloadWireSchema', () => {
  ...
  it('internalJobCode: rejects empty string (kills min(1) -> max(1) mutant)', () => {
    expect(() => ErpInvoicePayloadWireSchema.parse({ ...validPayload, internalJobCode: '' })).toThrow();
  });

  it('internalJobCode: accepts 64-char (max boundary, kills min(1).min(64) mutant)', () => {
    const sixtyfour = 'a'.repeat(64);
    expect(ErpInvoicePayloadWireSchema.parse({ ...validPayload, internalJobCode: sixtyfour }).internalJobCode).toBe(sixtyfour);
  });

  it('internalJobCode: rejects 65-char (kills max(64) -> min(64) mutant)', () => {
    const sixtyfive = 'a'.repeat(65);
    expect(() => ErpInvoicePayloadWireSchema.parse({ ...validPayload, internalJobCode: sixtyfive })).toThrow();
  });
  ...
  it('amountCents: accepts ERP_AMOUNT_CENTS_MAX exactly (kills max -> min mutant)', () => {
    expect(ErpInvoicePayloadWireSchema.parse({ ...validPayload, amountCents: ERP_AMOUNT_CENTS_MAX }).amountCents).toBe(ERP_AMOUNT_CENTS_MAX);
  });

  it('amountCents: rejects ERP_AMOUNT_CENTS_MAX + 1 (kills max -> min mutant from the other side)', () => {
    expect(() => ErpInvoicePayloadWireSchema.parse({ ...validPayload, amountCents: ERP_AMOUNT_CENTS_MAX + 1 })).toThrow();
  });

  it('amountCents: rejects non-integer (decimals)', () => {
    expect(() => ErpInvoicePayloadWireSchema.parse({ ...validPayload, amountCents: 100.5 })).toThrow();
  });
  ...
  it('erpSystem: accepts 64-char (kills min(1).min(64) mutant)', () => {
    ...
    expect(ErpInvoicePayloadWireSchema.parse({ ...validPayload, erpSystem: sixtyfour }).erpSystem).toBe(sixtyfour);
  });

  it('erpSystem: rejects 65-char (kills max(64) -> min(64) mutant)', () => {
    const sixtyfive = 'b'.repeat(65);
    expect(() => ErpInvoicePayloadWireSchema.parse({ ...validPayload, erpSystem: sixtyfive })).toThrow();
  });

  it('UUID fields: reject non-UUID strings', () => {
    expect(() => ErpInvoicePayloadWireSchema.parse({ ...validPayload, manifestCorrelationId: 'not-a-uuid' })).toThrow();
    expect(() => ErpInvoicePayloadWireSchema.parse({ ...validPayload, transportOrderId: 'not-a-uuid' })).toThrow();
    expect(() => ErpInvoicePayloadWireSchema.parse({ ...validPayload, internalCustomerId: 'not-a-uuid' })).toThrow();
  });
});
...
describe('ErpMappingContextWireSchema', () => {
  ...
  it('accepts null for both externalIds (union must allow null)', () => {
    const r = ErpMappingContextWireSchema.parse({ customerExternalId: null, jobCodeExternalId: null });
  ...
  it('customerExternalId: rejects empty string (kills min(1) -> max(1) mutant)', () => {
    expect(() => ErpMappingContextWireSchema.parse({ ...validMapping, customerExternalId: '' })).toThrow();
  });

  it('customerExternalId: accepts 128-char (kills max boundary mutants)', () => {
    const long = 'c'.repeat(128);
    expect(ErpMappingContextWireSchema.parse({ ...validMapping, customerExternalId: long }).customerExternalId).toBe(long);
  });

  it('customerExternalId: rejects 129-char (kills max(128) -> min(128) mutant)', () => {
    const tooLong = 'c'.repeat(129);
    expect(() => ErpMappingContextWireSchema.parse({ ...validMapping, customerExternalId: tooLong })).toThrow();
  });
  ...
  it('customerExternalId: rejects union shape mutated to []', () => {
    ...
    expect(() => ErpMappingContextWireSchema.parse({ ...validMapping, customerExternalId: 'ok' })).not.toThrow();
    expect(() => ErpMappingContextWireSchema.parse({ ...validMapping, customerExternalId: null })).not.toThrow();
  });

  it('jobCodeExternalId: rejects empty string + accepts 128-char + rejects 129-char (kills max/min mutants)', () => {
    expect(() => ErpMappingContextWireSchema.parse({ ...validMapping, jobCodeExternalId: '' })).toThrow();
    const long = 'd'.repeat(128);
    expect(ErpMappingContextWireSchema.parse({ ...validMapping, jobCodeExternalId: long }).jobCodeExternalId).toBe(long);
    const tooLong = 'd'.repeat(129);
    expect(() => ErpMappingContextWireSchema.parse({ ...validMapping, jobCodeExternalId: tooLong })).toThrow();
  });

  it('jobCodeExternalId: union accepts both string and null', () => {
    expect(() => ErpMappingContextWireSchema.parse({ ...validMapping, jobCodeExternalId: 'ok' })).not.toThrow();
    expect(() => ErpMappingContextWireSchema.parse({ ...validMapping, jobCodeExternalId: null })).not.toThrow();
  });
});
...
describe('ErpJobDataWireSchema', () => {
  ...
  it('rejects extra unknown keys at the top level (.strict())', () => {
    expect(() => ErpJobDataWireSchema.parse({ payload: validPayload, mapping: validMapping, extra: 'x' })).toThrow();
  });
});
