...
describe('AssignmentsClient', () => {
  it('GETs /transport-orders/assigned with bearer token', async () => {
    ...
    expect(fetchFn).toHaveBeenCalledWith(
      'http://api/transport-orders/assigned',
  ...
  it('rejects malformed response shape', async () => {
    const fetchFn = vi
      .fn()
      .mockResolvedValue({ ok: true, json: () => Promise.resolve({ rows: 'not-an-array' }) });
    const client = new AssignmentsClient({
      apiUrl: 'http://api',
      bearerToken: () => 't',
      fetchFn: fetchFn as never,
  ...
  it('rejects when a row is not an object', async () => {
    const fetchFn = vi
      .fn()
      .mockResolvedValue({ ok: true, json: () => Promise.resolve({ rows: ['not-obj'] }) });
    const { AssignmentsClient } = await import('../src/assignments/assignments-client.js');
    const client = new AssignmentsClient({
      apiUrl: 'http://api',
      bearerToken: () => 't',
  ...
  it('rejects when roadRunId is not a string', async () => {
    const fetchFn = vi.fn().mockResolvedValue({
      ok: true,
      json: () => Promise.resolve({ rows: [{ transportOrderId: 'to', roadRunId: 1 }] }),
    });
    const { AssignmentsClient } = await import('../src/assignments/assignments-client.js');
    const client = new AssignmentsClient({
      apiUrl: 'http://api',
  ...
  it('rejects when state is not a string', async () => {
    const fetchFn = vi.fn().mockResolvedValue({
      ok: true,
      json: () => Promise.resolve({ rows: [{ transportOrderId: 'to', roadRunId: 'r', state: 1 }] }),
    });
    const { AssignmentsClient } = await import('../src/assignments/assignments-client.js');
    const client = new AssignmentsClient({
      apiUrl: 'http://api',
...
describe('AssignmentsClient.completed', () => {
  ...
  it('GETs /transport-orders/completed with page + pageSize query and bearer token', async () => {
    ...
    expect(fetchFn).toHaveBeenCalledWith(
      'http://api/transport-orders/completed?page=1&pageSize=20',
      expect.objectContaining({
        method: 'GET',
        headers: expect.objectContaining({ Authorization: 'Bearer t0k' }),
      }),
