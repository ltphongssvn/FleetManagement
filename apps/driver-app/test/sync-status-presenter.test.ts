...
describe('@fleet/driver-app - presentSyncStatus', () => {
  ...
  it('shows backoff at THRESHOLD-1 failures, not circuit_open (#576)', () => {
    const v = presentSyncStatus(s({ consecutiveTransportFailures: 4, lastOutcome: 'last_transport_failure' }), NOW);
    expect(v.kind).toBe('backoff');
  });
  ...
  it('offline takes precedence over circuit_open (#577)', () => {
    const v = presentSyncStatus(s({ online: false, consecutiveTransportFailures: 10, lastOutcome: 'last_transport_failure' }), NOW);
    expect(v.kind).toBe('offline');
  });

  it('circuit_open requires lastOutcome=last_transport_failure (#593)', () => {
    const v = presentSyncStatus(s({ consecutiveTransportFailures: 10, lastOutcome: 'last_idle', lastSyncAtMs: NOW - 30_000 }), NOW);
    expect(v.kind).toBe('idle');
  });
    ...
    fc.assert(
      fc.property(
        fc.record({
          ...
          lastOutcome: fc.option(fc.constantFrom('last_idle', 'last_applied', 'last_transport_failure', 'last_storage_failure', 'last_protocol_violation', 'last_cursor_expired_recovered' as const)),
        ...
        (state, nowMs) => {
          ...
          expect(['idle', 'offline', 'app_inactive', 'backoff', 'circuit_open', 'never_synced']).toContain(v.kind);
          return true;
        },
      ),
    );
      ...
      fc.property(
        ...
        (failures, appActive, lastSync) => {
          const v = presentSyncStatus(
            { online: false, appActive, lastSyncAtMs: lastSync, lastOutcome: null, consecutiveTransportFailures: failures },
            1_700_000_000_000,
          );
          expect(v.kind).toBe('offline');
          return true;
    ...
    fc.assert(
      fc.property(
        ...
        (trigger, appActive, failures) => {
          const state = { online: false, appActive, lastSyncAtMs: null, lastOutcome: null, consecutiveTransportFailures: failures };
          const decision = decideSyncSchedule(state, trigger, 1_700_000_000_000, { random: () => 0.5 });
  ...
  it('circuit_open in scheduler implies circuit_open in presenter', () => {
    fc.assert(
      fc.property(
        ...
        (failures) => {
          const state = { online: true, appActive: true, lastSyncAtMs: 1_700_000_000_000 - 1_000, lastOutcome: 'last_transport_failure' as const, consecutiveTransportFailures: failures };
          const decision = decideSyncSchedule(state, 'timer_tick', 1_700_000_000_000, { random: () => 0.5 });
          const view = presentSyncStatus(state, 1_700_000_000_000);
