...
): SessionStoragePort & { current: StoredToken | null } {
  const box = {
    current: initial,
...
const LIVE: StoredToken = {
  ...
  refreshToken: 'rt-1',
  expiresAt: Math.floor(NOW_MS / 1000) + 600,
};
const NEAR_EXPIRY: StoredToken = {
  accessToken: 'old.jwt',
  refreshToken: 'rt-1',
...
describe('SessionManager.getAccessToken', () => {
  ...
  it('pre-refreshes inside the skew window: posts the stored refresh token, saves the rotated pair', async () => {
    ...
    expect(storage.current).toEqual({
      accessToken: 'new.jwt',
      refreshToken: 'rt-2',
      expiresAt: Math.floor(NOW_MS / 1000) + 900,
    });
  });
  ...
  it('refresh 401 (reuse/expired server-side) clears storage and throws SessionExpiredError', async () => {
    ...
    const fetchFn = vi
      .fn()
      .mockResolvedValue(jsonResponse(401, { title: 'unauthorized', status: 401 }));
    const sm = new SessionManager({
      apiUrl: API,
      fetchFn: fetchFn as never,
      storage,
  ...
  it('refresh 200 with a non-SSOT body fails CLOSED: clears storage and throws SessionExpiredError', async () => {
    ...
    const fetchFn = vi
      .fn()
      .mockResolvedValue(jsonResponse(200, { accessToken: 'x', expiresIn: 900 }));
    const sm = new SessionManager({
      apiUrl: API,
      fetchFn: fetchFn as never,
      storage,
...
describe('SessionManager.login', () => {
  ...
  it('non-SSOT 200 body (legacy access-only server) fails CLOSED as protocol-error, stores nothing', async () => {
    ...
    const fetchFn = vi
      .fn()
      .mockResolvedValue(jsonResponse(200, { accessToken: 'only.jwt', driver: LOGIN_OK.driver }));
    const sm = new SessionManager({
      apiUrl: API,
      fetchFn: fetchFn as never,
      storage,
  ...
  it('http 500 resolves http-error with the status for the presenter', async () => {
    ...
    await expect(sm.login('0901234567', 'pw')).resolves.toEqual({
      kind: 'http-error',
      status: 500,
    });
    expect(storage.current).toBeNull();
