...
function fnResolving(): { fn: (i: LifecycleMutationInput) => Promise<TransitionResult>; calls: LifecycleMutationInput[] } {
  const calls: LifecycleMutationInput[] = [];
  return {
    calls,
    fn: (i) => { calls.push(i); return Promise.resolve({ roadRunId: i.roadRunId, state: 'started' }); },
  };
}
...
describe('autoAdvanceAfterCapture', () => {
  it('fires start for a first photo on a planned run', async () => {
    ...
    const fired = await autoAdvanceAfterCapture({ roadRunId: 'rr-1', runState: 'planned', remainingBeforeThisUpload: 2 }, fn);
  ...
  it('fires complete when this photo was the last remaining stop', async () => {
    const { fn, calls } = fnResolving();
    const fired = await autoAdvanceAfterCapture({ roadRunId: 'rr-2', runState: 'started', remainingBeforeThisUpload: 1 }, fn);
  ...
  it('no-ops (null) when the policy yields no intent', async () => {
    const { fn, calls } = fnResolving();
    const fired = await autoAdvanceAfterCapture({ roadRunId: 'rr-3', runState: 'started', remainingBeforeThisUpload: 3 }, fn);
  ...
  it('swallows lifecycle errors and still reports the fired kind', async () => {
    ...
    const fired = await autoAdvanceAfterCapture({ roadRunId: 'rr-4', runState: 'planned', remainingBeforeThisUpload: 1 }, fn);
    expect(fired).toBe('complete');
  ...
  it('never fires for terminal or unknown states', async () => {
    const { fn, calls } = fnResolving();
    expect(await autoAdvanceAfterCapture({ roadRunId: 'rr-5', runState: 'cancelled', remainingBeforeThisUpload: 1 }, fn)).toBeNull();
    expect(await autoAdvanceAfterCapture({ roadRunId: 'rr-5', runState: 'weird', remainingBeforeThisUpload: 2 }, fn)).toBeNull();
    expect(calls).toEqual([]);
  });
});
