...
describe('@fleet/driver-app - runSpoolSweepOnBoot', () => {
  it('classifies entries within TTL as resume', async () => {
    ...
    const entries: readonly SpoolEntry[] = [
      baseEntry({ createdAtMs: now - 60_000, status: 'uploading' }),
    ];
    const remove = vi.fn().mockResolvedValue(undefined);
    const result = await runSpoolSweepOnBoot({
  ...
  it('abandons + removes entries past TTL', async () => {
    const now = Date.now();
    const entries: readonly SpoolEntry[] = [
      baseEntry({ createdAtMs: now - SPOOL_ENTRY_TTL_MS - 1000 }),
    ];
    const remove = vi.fn().mockResolvedValue(undefined);
    const result = await runSpoolSweepOnBoot({
      list: () => Promise.resolve(entries),
  ...
  it('skips in-progress entries without removing', async () => {
    const now = Date.now();
    const entries: readonly SpoolEntry[] = [
      baseEntry({ createdAtMs: now - 1000, status: 'uploading', attempts: 1 }),
    ];
    const remove = vi.fn().mockResolvedValue(undefined);
    const result = await runSpoolSweepOnBoot({
      list: () => Promise.resolve(entries),
