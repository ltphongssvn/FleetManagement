...
describe('@fleet/driver-app - notification setup barrel surface', () => {
  it('exports runNotificationSetup from the barrel', () => {
    expect(typeof (barrel as Record<string, unknown>)['runNotificationSetup'], 'the adapter and _layout call the policy through the barrel').toBe('function');
  });

  it('exports buildTransportAlertChannelConfig from the barrel', () => {
    expect(typeof (barrel as Record<string, unknown>)['buildTransportAlertChannelConfig'], 'the native adapter builds its channel input from the shared config builder').toBe('function');
  });

  it('the barrel-exported channel config carries the shared SSOT id and sound', () => {
    const build = (barrel as Record<string, unknown>)['buildTransportAlertChannelConfig'] as () => Record<string, unknown>;
    const cfg = build();
    expect(cfg['channelId'], 'channel id must come from the shared contract, not a local literal').toBe(DRIVER_ALERT_ANDROID_CHANNEL_ID);
    expect(cfg['sound'], 'sound must come from the shared contract so sender and channel agree').toBe(DRIVER_ALERT_SOUND);
  });
});

describe('@fleet/driver-app - notification-setup native adapter contract', () => {
  it('the native adapter module exists', () => {
    expect(() => src(ADAPTER), 'notification-setup-native.ts fills the NotificationPlatformPort seam').not.toThrow();
  });
  ...
  it('registers the channel at MAX importance (heads-up + sound)', () => {
    const s = src(ADAPTER);
    expect(s.includes('AndroidImportance.MAX'), 'below MAX there is no heads-up banner and no sound').toBe(true);
  });

  it('routes the tone through the ALARM audio usage (plays through a silenced ringer)', () => {
    const s = src(ADAPTER);
    expect(s.includes('AndroidAudioUsage.ALARM'), 'ALARM usage is what makes a phone on silent still wake the driver').toBe(true);
  });

  it('calls setNotificationChannelAsync to register the channel', () => {
    const s = src(ADAPTER);
    expect(s.includes('setNotificationChannelAsync'), 'the channel must actually be created, not merely described').toBe(true);
  });

  it('builds its channel input from the shared config builder, never local literals', () => {
    const s = src(ADAPTER);
    expect(s.includes('buildTransportAlertChannelConfig') || s.includes('config.channelId'), 'the adapter consumes the policy config; it does not restate the contract').toBe(true);
    expect(s.includes('transport-orders-v1'), 'a hardcoded channel id would drift from the api sender SSOT').toBe(false);
    expect(s.includes('transport_alert.wav'), 'a hardcoded sound filename would drift from the api sender SSOT').toBe(false);
  });

  it('installs a foreground handler that plays sound and shows a banner', () => {
    const s = src(ADAPTER);
    expect(s.includes('setNotificationHandler'), 'without a handler expo-notifications shows nothing while foregrounded').toBe(true);
  ...
  it('requests the Android permission through the port, not ad-hoc', () => {
    const s = src(ADAPTER);
    expect(s.includes('requestPermissionsAsync'), 'Android 13+ requires an explicit runtime request').toBe(true);
    expect(s.includes('getPermissionsAsync'), 'status must be read before prompting so a granted install never re-prompts').toBe(true);
  });

  it('gates the token fetch on a physical device', () => {
    const s = src(ADAPTER);
    expect(s.includes('isDevice'), 'expo-device isDevice gates getExpoPushTokenAsync; emulators yield no token').toBe(true);
