...
export default defineConfig({
  test: {
    ...
    coverage: {
      ...
      exclude: ['**/index.ts', '**/*.config.ts', '**/test/**', '**/schema.ts', 'src/storage/sqlite-sync-store.ts', 'src/storage/migrate.ts', 'src/sync/fetch-sync-transport.ts', 'src/push/notification-setup-native.ts', 'src/storage/native-bootstrap.ts', 'src/storage/native-bootstrap.web.ts', 'src/storage/sqlite-sync-store.web.ts', 'src/storage/migrate.web.ts', 'src/observability/sentry-bootstrap.ts', 'src/polyfills/install-fetch-polyfill.ts', 'src/auth/use-auth.tsx', 'src/assignments/use-trip-history.tsx', 'src/assignments/use-assignments.tsx', 'src/theme/tokens.ts'],
      thresholds: {
        statements: 90,
        branches: 90,
        functions: 90,
