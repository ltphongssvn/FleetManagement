...
describe('NaturalLanguageCreateForm (Mad-Libs sentence create form)', () => {
  ...
  it('renders inside a single form wired to the create action', async () => {
    const { NaturalLanguageCreateForm } = await import('@/features/dispatch/NaturalLanguageCreateForm');
  ...
  it('emits every FormData field name the unchanged action consumes', async () => {
    const { NaturalLanguageCreateForm } = await import('@/features/dispatch/NaturalLanguageCreateForm');
    render(<NaturalLanguageCreateForm drivers={drivers} locale={'vi'} />);
    for (const n of ['plannedStartAt', 'customer', 'cargo', 'assignedOperatorId', 'assignedAssetId', 'pickupAt', 'deliveryAt']) {
      expect(has('input[name=' + n + ']')).toBe(true);
    }
    expect(document.querySelectorAll('input[name^=pickupWarehouse_]').length).toBeGreaterThanOrEqual(1);
    expect(document.querySelectorAll('input[name^=deliveryWarehouse_]').length).toBeGreaterThanOrEqual(1);
  });
