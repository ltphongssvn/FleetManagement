...
function mkClient(
  over: Partial<DriversAdminClient>,
...
const assigned: AdminDriverRow = {
  ...
  phone: '0900000002',
  operatorId: 'op2',
  assignedVehicle: { vehicleId: 'v9', plate: '62H 05194' },
  assignmentId: 'as2',
  devices: [],
};
beforeEach(() => {
  globalThis.fetch = vi.fn().mockResolvedValue({
    ok: true,
    status: 200,
    json: () => Promise.resolve({ items: [{ id: 'v9', label: '62H 05194' }] }),
  }) as never;
});
afterEach(() => {
  vi.restoreAllMocks();
...
describe('DriversAdminSection mutations', () => {
  ...
  it('surfaces a create error via alert', async () => {
    const client = mkClient({ create: vi.fn().mockRejectedValue(new Error('Tài xế đã tồn tại')) }, [
      unassigned,
    ]);
    render(<DriversAdminSection client={client} />);
    const user = userEvent.setup();
  ...
  it('assigns a vehicle to an unassigned driver', async () => {
    ...
    await waitFor(() => {
      expect(client.assign).toHaveBeenCalledWith({ driverId: 'dr1', vehicleId: 'v9' });
    });
  });
  ...
  it('revokes an assignment after prompt', async () => {
    ...
    await waitFor(() => {
      expect(client.revoke).toHaveBeenCalledWith('as2', 'driver_left');
    });
  });
  it('deletes a driver after confirm', async () => {
    ...
    await waitFor(() => {
      expect(client.remove).toHaveBeenCalledWith('dr1');
    });
  });
  ...
  it('resets a password when the new value is valid', async () => {
    ...
    await waitFor(() => {
      expect(client.resetPassword).toHaveBeenCalledWith('dr1', 'newpass1');
    });
  });
  ...
  it('alerts when reset password fails', async () => {
    vi.spyOn(window, 'prompt').mockReturnValue('newpass1');
    const alertSpy = vi.spyOn(window, 'alert').mockImplementation(() => undefined);
    const client = mkClient({ resetPassword: vi.fn().mockRejectedValue(new Error('boom')) }, [
      unassigned,
    ]);
    render(<DriversAdminSection client={client} />);
    const user = userEvent.setup();
  ...
  it('edits the phone field before saving', async () => {
    ...
    await waitFor(() => {
      expect(client.update).toHaveBeenCalledWith('dr1', {
        fullName: 'NGUYEN VAN A',
        phone: '0999999999',
      });
    });
  ...
  it('renders a fully configured driver (assigned + device) in the table', async () => {
    const configured = {
      ...
      phone: '0900000003',
      operatorId: 'op3',
      assignedVehicle: { vehicleId: 'v3', plate: '62H 07777' },
      assignmentId: 'as3',
      devices: [{ deviceId: 'dev-abc' }],
    } as unknown as AdminDriverRow;
    const client = mkClient({}, [configured]);
    render(<DriversAdminSection client={client} />);
    await waitFor(() => {
      expect(screen.getByText('62H 07777')).toBeInTheDocument();
    });
  ...
  it('renders a configured driver in the regular table with revoke + device', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('51C-111.11')).toBeInTheDocument();
    });
    expect(screen.getByText('Đã đăng ký')).toBeInTheDocument();
    ...
    await user.click(await screen.findByRole('menuitem', { name: 'Hủy phân công' }));
    await waitFor(() => {
      expect(client.revoke).toHaveBeenCalledWith('asg-1', 'driver_left');
    });
  });
  ...
  it('tolerates a non-ok vehicles fetch (no vehicle options)', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('driver-assign-vehicle-dr1')).toBeInTheDocument();
    });
  });
  ...
  it('shows a device count suffix when a driver has more than one device', async () => {
    const twoDevices = {
      ...
      assignedVehicle: { vehicleId: 'v9', plate: '62H 08888' },
      assignmentId: 'as9',
      devices: [{ deviceId: 'dev-a' }, { deviceId: 'dev-b' }],
    } as unknown as AdminDriverRow;
