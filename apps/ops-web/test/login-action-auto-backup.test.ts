...
//
// L2: after a successful OIDC password-grant login, the action must
...
describe('@fleet/ops-web - login.action auto-backup wiring', () => {
  ...
  it('on successful login: POSTs to /transport-orders-export/auto with trigger=login', async () => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockImplementation((input) => {
      ...
      if (url.includes('/token')) {
        return Promise.resolve(new Response(JSON.stringify({ access_token: 'jwt-abc', expires_in: 3600 }), { status: 200 }) as never);
      }
      if (url.includes('/transport-orders-export/auto')) {
        return Promise.resolve(new Response(JSON.stringify({ exportLogId: 'log-1' }), { status: 200 }) as never);
      }
    ...
    const autoCalls = fetchSpy.mock.calls.filter((c) => {
      const url = typeof c[0] === 'string' ? c[0] : (c[0] as Request).url;
      return url.includes('/transport-orders-export/auto');
    });
    expect(autoCalls.length).toBe(1);
    const firstCall = autoCalls[0];
    if (!firstCall) throw new Error('no auto-backup call captured');
    const init: RequestInit | undefined = firstCall[1];
  ...
  it('auto-backup failure does NOT block the login', async () => {
    vi.spyOn(globalThis, 'fetch').mockImplementation((input) => {
      const url = typeof input === 'string' ? input : (input as Request).url;
      if (url.includes('/token')) {
        return Promise.resolve(new Response(JSON.stringify({ access_token: 'jwt-abc' }), { status: 200 }) as never);
      }
      return Promise.resolve(new Response('boom', { status: 500 }) as never);
    });
    const fd = new FormData();
    fd.set('username', 'dispatcher');
    fd.set('password', 'pw');
