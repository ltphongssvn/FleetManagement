...
describe('/api/admin/devices BFF routes', () => {
  ...
  it('GET forwards to /admin/devices preserving the status/page/pageSize query string', async () => {
    const fetchMock = backendFetch(
      JSON.stringify({ data: [], page: 1, pageSize: 20, total: 0, totalPages: 0, hasMore: false }),
    );
    vi.stubGlobal('fetch', fetchMock);
    const { GET } = (await import('@/app/api/admin/devices/route')) as unknown as {
      GET: RouteHandler;
    };
    const req = new NextRequest(
      new URL('https://ops.example.com/api/admin/devices?status=active&page=2&pageSize=5'),
    );
    const res = await GET(req);
    expect(res.status).toBe(200);
    const call = fetchMock.mock.calls.find((c) => String(c[0]).includes('/admin/devices'));
    expect(call).toBeDefined();
    expect(String((call as unknown[])[0])).toBe(
      API + '/admin/devices?status=active&page=2&pageSize=5',
    );
  ...
  it('PATCH :deviceId/binding forwards method + JSON body to the API binding path', async () => {
    ...
    vi.stubGlobal('fetch', fetchMock);
    const { PATCH } =
      (await import('@/app/api/admin/devices/[deviceId]/binding/route')) as unknown as {
        PATCH: RouteHandler;
      };
    const deviceId = '00000000-0000-0000-0000-0000000000d1';
    const payload = JSON.stringify({ action: 'revoke', revokedReason: 'stolen' });
    const req = new NextRequest(
      new URL('https://ops.example.com/api/admin/devices/' + deviceId + '/binding'),
      {
        method: 'PATCH',
        body: payload,
