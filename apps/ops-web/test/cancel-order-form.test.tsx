...
describe('CancelOrderForm', () => {
  it('renders the open button when the order state is draft', () => {
    defaultMock();
    render(
      <CancelOrderForm transportOrderId="11111111-1111-1111-1111-111111111111" state="draft" />,
    );
  ...
  it('renders the open button when the order state is assigned', () => {
    defaultMock();
    render(
      <CancelOrderForm transportOrderId="11111111-1111-1111-1111-111111111111" state="assigned" />,
    );
    expect(screen.getByTestId('order-cancel-open')).toBeTruthy();
  });
  ...
  it('does NOT render the open button when the order state is already cancelled', () => {
    defaultMock();
    render(
      <CancelOrderForm transportOrderId="11111111-1111-1111-1111-111111111111" state="cancelled" />,
    );
  ...
  it('does NOT render the open button when the order state is completed', () => {
    defaultMock();
    render(
      <CancelOrderForm transportOrderId="11111111-1111-1111-1111-111111111111" state="completed" />,
    );
    expect(screen.queryByTestId('order-cancel-open')).toBeNull();
  });
  ...
  it('renders the conflict message when the action returns status=conflict', async () => {
    cancelOrderMock.mockImplementation(() =>
      Promise.resolve({ status: 'conflict', message: 'cannot cancel' } satisfies CancelOrderState),
    );
    openFormAndSubmit();
  ...
  it('renders the not_found message when the action returns status=not_found', async () => {
    cancelOrderMock.mockImplementation(() =>
      Promise.resolve({ status: 'not_found', message: 'gone' } satisfies CancelOrderState),
    );
    openFormAndSubmit();
  ...
  it('falls through invalid.errors -> errors.transportOrderId when reason and note are missing', async () => {
    cancelOrderMock.mockImplementation(() =>
      Promise.resolve({
        status: 'invalid',
        errors: { transportOrderId: 'Bad id' },
      } satisfies CancelOrderState),
    );
    openFormAndSubmit();
  ...
  it('falls through to the generic invalid message when no specific field error is present', async () => {
    cancelOrderMock.mockImplementation(() =>
      Promise.resolve({ status: 'invalid', errors: {} } satisfies CancelOrderState),
    );
    openFormAndSubmit();
  ...
  it('requires a note when reason=other: submit disabled and warning shown until a note is typed', () => {
    ...
    fireEvent.change(screen.getByTestId('order-cancel-note'), {
      target: { value: 'khách đổi lịch' },
    });
...
describe('CancelOrderForm - photos-received guard (server-computed affordance)', () => {
  it('does NOT render the open button when canCancel is false', () => {
    defaultMock();
    render(
      <CancelOrderForm
        transportOrderId="11111111-1111-1111-1111-111111111111"
  ...
  it('shows an explanatory blocked message when canCancel is false with photos_received', () => {
    defaultMock();
    render(
      <CancelOrderForm
        transportOrderId="11111111-1111-1111-1111-111111111111"
    ...
    const blocked = screen.getByTestId('order-cancel-blocked');
    expect(blocked).toBeTruthy();
    expect(blocked.textContent).toContain('phiếu cân');
  });
  it('still renders the open button when canCancel is true (default behavior preserved)', () => {
    defaultMock();
    render(
      <CancelOrderForm
        transportOrderId="11111111-1111-1111-1111-111111111111"
  ...
  it('defaults to cancellable when canCancel prop is omitted (back-compat)', () => {
    defaultMock();
    render(
      <CancelOrderForm transportOrderId="11111111-1111-1111-1111-111111111111" state="draft" />,
    );
