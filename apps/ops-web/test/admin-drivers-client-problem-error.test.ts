...
describe('AdminDriversClient throws ApiProblemError the presenter can map', () => {
  ...
  it('happy path unchanged: list() on 200 returns rows without throwing', async () => {
    // Wire-truthful row: list() now parses at the boundary (parseAdminDriverRows),
    // so a 200 body must satisfy AdminDriverRowSchema, not just carry driverId.
    // The old driverId-only stub passed only because the client used to CAST.
    const rows = [{
      driverId: 'd1',
