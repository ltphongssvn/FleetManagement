...
// Bilingual VN/EN dispatcher form mirroring the LỆNH ĐIỀU XE spreadsheet.
// Uses Headless UI Combobox via ComboboxField — portal-rendered, searchable, immune to overflow clipping.
'use client';
...
export function CreateOrderForm({
  ...
  const ph = (k: string): string => locale === 'vi' ? `— Chọn ${k.toLowerCase()} —` : `— Select ${k.toLowerCase()} —`;
  ...
  return (
    <form action={formAction} className="rounded-2xl border border-white/60 bg-white/95 shadow-xl shadow-indigo-900/5 ring-1 ring-slate-900/5">
      ...
      <div className={sectionCls}>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <input id="plannedStartAt" name="plannedStartAt" type="datetime-local" required className={`${inputCls} mt-1`} />
          </div>
        </div>
      </div>
      ...
      <div className={sectionCls}>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            ...
            <input id="pickupAt" name="pickupAt" type="datetime-local" required className={`${inputCls} mt-1`} />
      ...
      <div className={sectionCls}>
        <div className={sectionTitleCls}><span className={stepBadgeCls}>5</span>{tx('orderForm.deliveryDate')} & {tx('orderForm.deliveryWarehouse')}</div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <label htmlFor="deliveryAt" className={labelCls}>{tx('orderForm.deliveryDate')}</label>
            <input id="deliveryAt" name="deliveryAt" type="datetime-local" required className={`${inputCls} mt-1`} />
            <FieldError msg={errs.deliveryAt} />
          </div>
          <div>
            <label htmlFor="deliveryWarehouse" className={labelCls}>{tx('orderForm.deliveryWarehouse')}</label>
            <ComboboxField id="deliveryWarehouse" name="deliveryWarehouse" options={deliveryWarehouses} placeholder={ph(tx('orderForm.deliveryWarehouse'))} />
          </div>
        </div>
      </div>
