...
// allocated XTT.MM-NNN in a success banner. The defaultOrderRef prop is kept on
...
export function CreateOrderForm({
  ...
  return (
    <form
      ...
      className="rounded-2xl border border-white/60 bg-white/95 shadow-xl shadow-indigo-900/5 ring-1 ring-slate-900/5"
    >
      <div className="rounded-t-2xl border-b border-slate-200/70 bg-gradient-to-r from-indigo-50/60 via-white/40 to-sky-50/60 px-6 py-4">
        ...
        <p className="mt-0.5 text-xs text-slate-500">
          {locale === 'vi'
            ...
            : 'Fill in the fields below to create a dispatch order.'}
        </p>
      </div>
      {topError ? (
        <div
      ...
      {createdRef ? (
        <div
          role="status"
          className="mx-6 mt-4 rounded-md border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800"
        >
          <span className="font-semibold">{tx('orderForm.orderNo')}:</span>{' '}
      ...
      <div className={sectionCls}>
        <div className={sectionTitleCls}>
          ...
          {tx('orderForm.orderNo')} & {tx('orderForm.orderDate')}
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <label className={labelCls}>{tx('orderForm.orderNo')}</label>
            <div
              data-testid="order-no-readonly"
              className={inputCls + ' mt-1 font-mono uppercase bg-slate-50 text-slate-500'}
            >
              {createdRef ?? (locale === 'vi' ? 'Hệ thống tự cấp' : 'Auto-assigned')}
            </div>
          </div>
          <div>
            <label htmlFor="plannedStartAt" className={labelCls}>
              {tx('orderForm.orderDate')}
            </label>
            <input
              id="plannedStartAt"
              name="plannedStartAt"
      ...
      <div className={sectionCls}>
        <div className={sectionTitleCls}>
          ...
          {tx('orderForm.customer')} & {tx('orderForm.cargo')}
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <label htmlFor="customer" className={labelCls}>
              {tx('orderForm.customer')}
            </label>
            <ComboboxField
              id="customer"
              name="customer"
              options={customers}
              placeholder={ph(tx('orderForm.customer'))}
              submitValue="id"
            />
          </div>
          <div>
            ...
            <ComboboxField
              ...
              name="cargo"
              options={cargoTypes}
              placeholder={ph(tx('orderForm.cargo'))}
              submitValue="id"
            />
          </div>
        </div>
      ...
      <div className={sectionCls}>
        <div className={sectionTitleCls}>
          ...
          {tx('orderForm.vehiclePlate')} / {tx('orderForm.driverName')}
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <label htmlFor="vehiclePlate" className={labelCls}>
              {tx('orderForm.vehiclePlate')}
            </label>
            <ComboboxField
              id="vehiclePlate"
              name="vehiclePlate"
          ...
          <div>
            <label htmlFor="assignedOperatorId" className={labelCls}>
              {tx('orderForm.driverName')}
            </label>
            <ComboboxField
      ...
      <div className={sectionCls}>
        ...
        <div className="mb-4">
          <label htmlFor="pickupAt" className={labelCls}>
            {tx('orderForm.pickupDate')}
          </label>
          ...
          {pickupRows.map((n) => {
            ...
            return (
              <div key={n} className="rounded-lg border border-slate-200 bg-slate-50/60 p-3">
                <label htmlFor={whId} className={labelCls}>
                  {tx('orderForm.pickup')} {n}
                </label>
                <ComboboxField
                  id={whId}
                  name={whId}
                  options={pickupWarehouses}
                  placeholder={tx('orderForm.none')}
                  submitValue="id"
                />
              </div>
            );
      ...
      <div className={sectionCls}>
        ...
        <div className="mb-4">
          <label htmlFor="deliveryAt" className={labelCls}>
            {tx('orderForm.deliveryDate')}
          </label>
              ...
              <div key={n} className="rounded-lg border border-slate-200 bg-slate-50/60 p-3">
                <label htmlFor={whId} className={labelCls}>
                  {deliveryRows.length === 1
                    ? tx('orderForm.deliveryWarehouse')
                    : tx('orderForm.deliveryWarehouse') + ' ' + String(n)}
                </label>
                <ComboboxField
                  id={whId}
                  name={whId}
      ...
      <div className="flex items-center justify-between gap-3 rounded-b-2xl border-t border-slate-200/70 bg-gradient-to-r from-slate-50/80 to-white/60 px-6 py-4">
        <p className="text-xs text-slate-500">
          {locale === 'vi'
            ? 'Kiểm tra kỹ thông tin trước khi tạo lệnh.'
