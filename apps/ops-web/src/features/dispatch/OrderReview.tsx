...
function StopStatusCell({
  stop,
  index,
}: {
  stop: ListAssignedRowStop;
  index: number;
}): JSX.Element {
    ...
    <section className="mx-auto w-full max-w-3xl rounded-2xl bg-white/95 p-6 shadow-sm">
      ...
      <dl className="mt-6 grid grid-cols-1 gap-x-6 gap-y-3 text-sm sm:grid-cols-2">
        <div>
          ...
          <dd data-testid="order-review-external-ref" className="mt-1 text-slate-900">
            {dash(order.externalRef)}
          </dd>
        </div>
        <div>
          <dt className="font-medium text-slate-500">Biển số xe</dt>
          <dd data-testid="order-review-plate" className="mt-1 text-slate-900">
            {dash(order.plate)}
          </dd>
        </div>
        <div>
          <dt className="font-medium text-slate-500">Tài xế</dt>
          <dd data-testid="order-review-driver" className="mt-1 text-slate-900">
            {dash(order.driverName)}
          </dd>
        </div>
        <div>
          <dt className="font-medium text-slate-500">Khách hàng</dt>
          <dd data-testid="order-review-customer" className="mt-1 text-slate-900">
            {dash(order.customerName)}
          </dd>
        </div>
        <div>
          <dt className="font-medium text-slate-500">Tên hàng</dt>
          <dd data-testid="order-review-cargo" className="mt-1 text-slate-900">
            {dash(order.cargoName)}
          </dd>
        </div>
        <div>
          <dt className="font-medium text-slate-500">Ngày tạo lệnh</dt>
          <dd data-testid="order-review-created-at" className="mt-1 text-slate-900">
            {formatDateTime(order.createdAt)}
          </dd>
        </div>
        <div>
          <dt className="font-medium text-slate-500">Trạng thái</dt>
          <dd data-testid="order-review-state" className="mt-1 text-slate-900">
            {order.state}
          </dd>
        </div>
      </dl>
      <div className="mt-8">
        ...
        <div
          data-testid="order-review-stops-header"
          className="mt-3 grid grid-cols-12 gap-2 border-b border-slate-200 px-4 py-2 text-xs font-semibold uppercase tracking-wide text-slate-500"
        >
          <span className="col-span-4">Điểm</span>
          <span className="col-span-4">Kho</span>
          <span data-testid="order-review-stops-date-header" className="col-span-2">
            Ngày dự kiến
          </span>
          <span className="col-span-2">Trạng thái</span>
        </div>
        <ol
          data-testid="order-review-stops"
          className="divide-y divide-slate-200 rounded-b-lg border border-t-0 border-slate-200"
        >
          {order.stops.map((s, i) => (
            <li
              key={s.sequence}
              data-testid="order-review-stop"
            ...
            >
              ...
              <span data-testid="order-review-stop-warehouse" className="col-span-4 text-slate-700">
                {dash(s.warehouseName)}
              </span>
              <span className="col-span-2 text-slate-500">{formatDateTime(s.plannedAt)}</span>
              <span className="col-span-2 text-slate-700">
                <StopStatusCell stop={s} index={i} />
              </span>
            </li>
          ))}
        </ol>
