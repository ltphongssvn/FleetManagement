...
async function buildAttestation(opts: AttestOptions): Promise<AttestArtifacts> {
  ...
  const attStmt = new Map<string, CBORType>([
    ['x5c', x5c as unknown as CBORType],
...
describe('verifyIosAppAttest', () => {
  it('accepts a valid production attestation and reports environment + key', async () => {
    const a = await buildAttestation({
      challenge: 'nonce-1',
      counter: 0,
      aaguidSeed: PROD_AAGUID,
      root: appleRootCert,
      rootKeys: appleRootKeys,
      tamperNonce: false,
      wrongKeyId: false,
    });
  ...
  it('rejects an rpIdHash from the wrong team/bundle', async () => {
    ...
    const out = await verifyIosAppAttest(a.attestationObject, {
      keyId: a.keyId,
      clientDataHash: a.clientDataHash,
      expectedTeamId: 'ZZZZZ99999',
      expectedBundleId: BUNDLE_ID,
      now: NOW,
      isTrustedRoot: trustOnly(appleRootCert),
    });
  ...
  it('rejects an object whose attStmt has no x5c', async () => {
    ...
    const obj = new Map<string, CBORType>([
      ['fmt', 'apple-appattest'],
      ['attStmt', attStmt as unknown as CBORType],
      ['authData', new Uint8Array(60)],
    ]);
    const bytes = cborEncode(obj);
    const out = await verifyIosAppAttest(bytes, {
  ...
  it('rejects an x5c entry that is not bytes', async () => {
    const attStmt = new Map<string, CBORType>([
      ['x5c', ['not-bytes'] as unknown as CBORType],
      ['receipt', new Uint8Array(0)],
    ]);
    const obj = new Map<string, CBORType>([
      ['fmt', 'apple-appattest'],
  ...
  it('rejects a credCert extension whose CBOR is not an octet string', async () => {
    ...
    const credCert = await x509.X509CertificateGenerator.create({
      serialNumber: '02',
      subject: 'CN=Cred',
      issuer: appleRootCert.subject,
      notBefore: new Date('2024-06-01T00:00:00Z'),
      notAfter: new Date('2035-01-01T00:00:00Z'),
      signingAlgorithm: ALG,
      publicKey: credKeys.publicKey,
      signingKey: appleRootKeys.privateKey,
      extensions: [new x509.Extension(APP_ATTEST_OID, false, nonNumberCbor)],
    });
    ...
    const out = await verifyIosAppAttest(bytes, {
      keyId: sha256(spki),
      clientDataHash: sha256(new TextEncoder().encode('x')),
      expectedTeamId: TEAM_ID,
      expectedBundleId: BUNDLE_ID,
      now: NOW,
      isTrustedRoot: trustOnly(appleRootCert),
  ...
  it('rejects an x5c entry that is bytes but not a valid certificate', async () => {
    ...
    const attStmt = new Map<string, CBORType>([
      ['x5c', [bogusCert] as unknown as CBORType],
      ['receipt', new Uint8Array(0)],
    ]);
    const obj = new Map<string, CBORType>([
      ['fmt', 'apple-appattest'],
    ...
    const out = await verifyIosAppAttest(cborEncode(obj), {
      keyId: new Uint8Array(32),
      clientDataHash: new Uint8Array(32),
      expectedTeamId: TEAM_ID,
      expectedBundleId: BUNDLE_ID,
  ...
  it('rejects a credCert extension whose bytes are not decodable CBOR', async () => {
    ...
    const credCert = await x509.X509CertificateGenerator.create({
      ...
      publicKey: credKeys.publicKey,
      signingKey: appleRootKeys.privateKey,
      extensions: [new x509.Extension(APP_ATTEST_OID, false, undecodable)],
    });
    const x5c = [new Uint8Array(credCert.rawData), new Uint8Array(appleRootCert.rawData)];
    const attStmt = new Map<string, CBORType>([
      ['x5c', x5c as unknown as CBORType],
  ...
  it('rejects a chain whose leaf signature does not verify against the issuer', async () => {
    ...
    const credCert = await x509.X509CertificateGenerator.create({
      ...
      extensions: [
        new x509.Extension(APP_ATTEST_OID, false, cborEncode(nonce as unknown as CBORType)),
      ],
    });
    const x5c = [new Uint8Array(credCert.rawData), new Uint8Array(appleRootCert.rawData)];
    const attStmt = new Map<string, CBORType>([
  ...
  it('rejects malformed cbor', async () => {
    const out = await verifyIosAppAttest(new Uint8Array([0xff, 0x00, 0x13]), {
      keyId: new Uint8Array(32),
      clientDataHash: new Uint8Array(32),
      expectedTeamId: TEAM_ID,
      expectedBundleId: BUNDLE_ID,
