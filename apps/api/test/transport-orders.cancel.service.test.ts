...
// thenable that resolves to [] (no linked runs in unit-test mode) AND
// exposes .limit() so the original existence-lookup shape keeps working.
// Integration tests cover the real cascade against PGlite.
...
interface MakeDbOpts {
  ...
  updatedRow?: FakeOrderRow;
}
function makeDb(opts: MakeDbOpts): { db: unknown; updateValues: ReturnType<typeof vi.fn>; roadRunUpdateCalled: { value: boolean } } {
  const updateValues = vi.fn();
  const roadRunUpdateCalled = { value: false };
  const txObject = {
    select: () => ({
      from: () => {
        const limitable = {
    ...
    update: () => ({
      set: (v: unknown) => {
        ...
        if (isCascade) {
          roadRunUpdateCalled.value = true;
          return {
  ...
  return { db, updateValues, roadRunUpdateCalled };
}
