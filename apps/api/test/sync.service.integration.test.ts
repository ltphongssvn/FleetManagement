...
describe('@fleet/api - SyncService (integration)', () => {
  ...
  it('writes to all three append paths in one tx', async () => {
    const res = await service.processSync(
      {
        ...
        actions: [makeAction('00000000-0000-0000-0000-000000000aa1')],
      },
      OP,
    );
    ...
    const audit = await testDb.db.execute<{ count: string }>(
      sql`SELECT COUNT(*)::text as count FROM fleet_audit_log`,
    );
    const feed = await testDb.db.execute<{ count: string }>(
      sql`SELECT COUNT(*)::text as count FROM sync_change_feed`,
    );
    const outboxCount = await testDb.db.execute<{ count: string }>(
      sql`SELECT COUNT(*)::text as count FROM outbox`,
    );
    ...
    const res = await service.processSync(
      {
        cursor: createSyncCursor('0'),
        actions: [
          makeAction('00000000-0000-0000-0000-000000000aa6'),
          makeAction('00000000-0000-0000-0000-000000000aa7'),
        ],
      },
      OP,
    );
    const maxRow = await testDb.db.execute<{ max_seq: string }>(
      sql`SELECT MAX(server_seq)::text AS max_seq FROM sync_change_feed`,
    );
  ...
  it('isolates by company_id (no cross-tenant leak)', async () => {
    const otherTenant: OperatorContext = {
      ...OP,
      companyId: '00000000-0000-0000-0000-0000000000ff',
    };
    await service.processSync(
      {
        cursor: createSyncCursor('0'),
        actions: [makeAction('00000000-0000-0000-0000-000000000ab1')],
      },
      OP,
    );
  ...
  it('keeps results aligned with input action order across mixed outcomes (applied/duplicate)', async () => {
    ...
    const res = await service.processSync(
      { cursor: createSyncCursor('0'), actions: [a, b, c] },
      OP,
    );
  ...
  it('continues processing later actions when an earlier action is duplicate', async () => {
    ...
    const res = await service.processSync(
      { cursor: createSyncCursor('0'), actions: [dup, fresh] },
      OP,
    );
  ...
  it('isolates by full tenancy tuple (companyId+businessUnitId+depotId+legalEntityId)', async () => {
    ...
    await service.processSync(
      {
        cursor: createSyncCursor('0'),
        actions: [makeAction('00000000-0000-0000-0000-000000000c01')],
      },
      OP,
    );
    await service.processSync(
      {
        cursor: createSyncCursor('0'),
        actions: [makeAction('00000000-0000-0000-0000-000000000c02')],
      },
      sameCompanyDifferentDepot,
    );
  ...
  it('rolls back all 3 append paths atomically when one insert fails (no orphan audit/feed/outbox)', async () => {
    ...
    const baselineAudit = await testDb.db.execute<{ count: string }>(
      sql`SELECT COUNT(*)::text as count FROM fleet_audit_log`,
    );
    const baselineFeed = await testDb.db.execute<{ count: string }>(
      sql`SELECT COUNT(*)::text as count FROM sync_change_feed`,
    );
    const baselineOutbox = await testDb.db.execute<{ count: string }>(
      sql`SELECT COUNT(*)::text as count FROM outbox`,
    );
    ...
    const finalAudit = await testDb.db.execute<{ count: string }>(
      sql`SELECT COUNT(*)::text as count FROM fleet_audit_log`,
    );
    const finalFeed = await testDb.db.execute<{ count: string }>(
      sql`SELECT COUNT(*)::text as count FROM sync_change_feed`,
    );
    const finalOutbox = await testDb.db.execute<{ count: string }>(
      sql`SELECT COUNT(*)::text as count FROM outbox`,
    );
