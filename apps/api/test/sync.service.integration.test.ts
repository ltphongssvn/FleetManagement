...
describe('@fleet/api - SyncService (integration)', () => {
  ...
  it('logs duplicate at debug level (structured observability)', async () => {
    ...
    await service.processSync({ cursor: '0', actions: [action] }, OP);
    await service.processSync({ cursor: '0', actions: [action] }, OP);
  ...
  it('returns full SyncResponse contract on empty request (wire-shape guarantee)', async () => {
    const res = await service.processSync({ cursor: '0', actions: [] }, OP);
    expect(res).toMatchObject({
      status: 'ok',
      results: [],
      newCursor: '0',
  ...
  it('keeps results aligned with input action order across mixed outcomes (applied/duplicate)', async () => {
    ...
    await service.processSync({ cursor: '0', actions: [b] }, OP);
    const res = await service.processSync({ cursor: '0', actions: [a, b, c] }, OP);
  ...
  it('continues processing later actions when an earlier action is duplicate', async () => {
    ...
    await service.processSync({ cursor: '0', actions: [dup] }, OP);
    const fresh = makeAction('00000000-0000-0000-0000-000000000b02');
    const res = await service.processSync({ cursor: '0', actions: [dup, fresh] }, OP);
  ...
  it('isolates by full tenancy tuple (companyId+businessUnitId+depotId+legalEntityId)', async () => {
    ...
    await service.processSync({ cursor: '0', actions: [makeAction('00000000-0000-0000-0000-000000000c01')] }, OP);
    await service.processSync({ cursor: '0', actions: [makeAction('00000000-0000-0000-0000-000000000c02')] }, sameCompanyDifferentDepot);
  ...
  it('rolls back all 3 append paths atomically when one insert fails (no orphan audit/feed/outbox)', async () => {
    ...
    await service.processSync({ cursor: '0', actions: [a1] }, OP);
    ...
    const res = await service.processSync({ cursor: '0', actions: [dup] }, OP);
