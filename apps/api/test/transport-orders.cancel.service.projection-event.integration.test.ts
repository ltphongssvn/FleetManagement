...
async function seedActivePair(tx: TestTx, op: ReturnType<typeof createOperatorContext>): Promise<{ operatorId: string; vehicleId: string }> {
  ...
  const [d] = await tx.insert(driver).values({ ...tn, fullName: 'ProjectionEventDriver', operatorId: op.operatorId }).returning({ driverId: driver.driverId });
  const [v] = await tx.insert(vehicle).values({ ...tn, plate: 'PE-' + randomUUID().slice(0, 4) }).returning({ vehicleId: vehicle.vehicleId });
...
describe('@fleet/api - TransportOrdersCancelService publishes sync_change_feed event for projection', () => {
  ...
  it('appends a road_run sync_change_feed event with delta.state=cancelled when a transport_order is cancelled', async () => {
    ...
    let cancellationEvents: readonly { aggregateType: string; aggregateId: string; delta: unknown }[] = [];
    await withTxIsolation(testDb, async (tx) => {
      ...
      const created = await createSvc.create({
        ...
        roadRun: { plannedStartAt: '2026-06-10T07:00:00.000Z', assignedOperatorId: operatorId, assignedAssetId: vehicleId },
      }, op);
    ...
    const roadRunEvents = cancellationEvents.filter((e) => e.aggregateType === 'road_run' && e.aggregateId === createdRoadRunId);
    ...
    expect(cancelledEvent, 'expected a road_run event with delta.state=cancelled to be appended').toBeTruthy();
  });
});
