...
// accept BOTH a UUID and an XT.NNNN external_ref. Dispatch board rows now
...
describe('@fleet/api - TransportOrdersReviewController.findOne (T5 ref-or-id)', () => {
  ...
  it('passes an XT.NNNN external_ref through to findByCompanyIdOrRef', async () => {
    const op = createOperatorContext();
    const svc = { findByCompanyIdOrRef: vi.fn().mockResolvedValue(sampleRow) };
    const ctl = new TransportOrdersReviewController(svc as never);
  ...
  it('rejects a garbage param that is neither a UUID nor an XT.NNNN ref', async () => {
    const op = createOperatorContext();
