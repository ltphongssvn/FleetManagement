...
// accept BOTH a UUID and an XTT.MM-NNN external_ref. Dispatch board rows now
...
describe('@fleet/api - TransportOrdersReviewController.findOne (T5 ref-or-id)', () => {
  ...
  it('rejects a garbage param that is neither a UUID nor an XTT.MM-NNN ref', async () => {
    const op = createOperatorContext();
