...
async function seedActivePair(tx: TestTx, op: ReturnType<typeof createOperatorContext>): Promise<{
  ...
  const [d] = await tx.insert(driver).values({ ...tn, fullName: 'ALERT-INT', operatorId })
    .returning({ driverId: driver.driverId });
  const [v] = await tx.insert(vehicle).values({ ...tn, plate: 'ALR-' + operatorId.slice(0, 4) })
...
describe('@fleet/api - TransportOrdersService driver-alert outbox emission (integration)', () => {
  ...
  it('create() enqueues exactly one driver_alert.requested outbox row on the alerts queue, atomic with the order', async () => {
    await withTxIsolation(testDb, async (tx) => {
      ...
      const alertRows = await tx.execute<{ payload: string }>(sql.raw(
        "SELECT payload::text as payload FROM outbox WHERE queue_name = 'alerts'",
      ));
  ...
  it('does not add alert rows to sync_change_feed or fleet_audit_log (plain outbox insert, not tri-write)', async () => {
    await withTxIsolation(testDb, async (tx) => {
      ...
      const feed = await tx.execute<{ count: string }>(sql.raw(
        "SELECT COUNT(*)::text as count FROM sync_change_feed WHERE aggregate_type = 'driver_alert'",
      ));
      const audit = await tx.execute<{ count: string }>(sql.raw(
        "SELECT COUNT(*)::text as count FROM fleet_audit_log WHERE aggregate_type = 'driver_alert'",
      ));
      const projections = await tx.execute<{ count: string }>(sql.raw(
        "SELECT COUNT(*)::text as count FROM outbox WHERE queue_name = 'projections'",
      ));
