  ...
  describe('drivers(op)', () => {
    it('returns paired drivers and excludes an unpaired driver seeded alongside', async () => {
      const result = await withTxIsolation(testDb, async (tx) => {
        ...
        const [drvP] = await tx.insert(driver).values({
          ...tenancy(op), fullName: 'AAA PairedDrv', active: true, operatorId: opPaired,
        }).returning();
        await tx.insert(driver).values({
          ...tenancy(op), fullName: 'BBB UnpairedDrv', active: true, operatorId: opUnpaired,
        });
        const [vehP] = await tx.insert(vehicle).values({
          ...tenancy(op), plate: 'PR-01', active: true,
        }).returning();
        if (!drvP || !vehP) throw new Error('seed failed');
        await tx.insert(driverVehicleAssignment).values({
          ...tenancy(op), driverId: drvP.driverId, vehicleId: vehP.vehicleId,
        });
        return (await svc.drivers(op)).items;
      });
      expect(result).toEqual([{ id: '00000000-0000-0000-0000-000000001001', label: 'AAA PairedDrv' }]);
    });
    it('excludes a driver whose only assignment is revoked while keeping a sibling paired driver', async () => {
      const result = await withTxIsolation(testDb, async (tx) => {
        ...
        const [drvOk] = await tx.insert(driver).values({
          ...tenancy(op), fullName: 'AAA OkDrv', active: true, operatorId: opOk,
        }).returning();
        const [drvRev] = await tx.insert(driver).values({
          ...tenancy(op), fullName: 'BBB RevDrv', active: true, operatorId: opRev,
        }).returning();
        const [vehOk] = await tx.insert(vehicle).values({
          ...tenancy(op), plate: 'OK-01', active: true,
        }).returning();
        const [vehRev] = await tx.insert(vehicle).values({
          ...tenancy(op), plate: 'RV-01', active: true,
        }).returning();
        if (!drvOk || !drvRev || !vehOk || !vehRev) throw new Error('seed failed');
        await tx.insert(driverVehicleAssignment).values([
          { ...tenancy(op), driverId: drvOk.driverId, vehicleId: vehOk.vehicleId },
          { ...tenancy(op), driverId: drvRev.driverId, vehicleId: vehRev.vehicleId, revokedAt: new Date() },
        ]);
        return (await svc.drivers(op)).items;
      });
    ...
    it('excludes a driver paired only with an inactive vehicle while keeping a sibling paired driver', async () => {
      const result = await withTxIsolation(testDb, async (tx) => {
        ...
        const [drvOk] = await tx.insert(driver).values({
          ...tenancy(op), fullName: 'AAA OkDrv2', active: true, operatorId: opOk,
        }).returning();
        const [drvIV] = await tx.insert(driver).values({
          ...tenancy(op), fullName: 'BBB IVDrv', active: true, operatorId: opIV,
        }).returning();
        const [vehOk] = await tx.insert(vehicle).values({
          ...tenancy(op), plate: 'OK-02', active: true,
        }).returning();
        const [vehIV] = await tx.insert(vehicle).values({
          ...tenancy(op), plate: 'IV-01', active: false,
        }).returning();
    ...
    it('excludes pairings whose joined vehicle belongs to another company while keeping in-company pair', async () => {
      const result = await withTxIsolation(testDb, async (tx) => {
        ...
        const [drvOk] = await tx.insert(driver).values({
          ...tenancy(opMine), fullName: 'AAA OkDrv3', active: true, operatorId: opIdOk,
        }).returning();
        const [drvBad] = await tx.insert(driver).values({
          ...tenancy(opMine), fullName: 'BBB BadDrv', active: true, operatorId: opIdBad,
        }).returning();
        const [vehOk] = await tx.insert(vehicle).values({
          ...tenancy(opMine), plate: 'OK-03', active: true,
        }).returning();
        const [vehForeign] = await tx.insert(vehicle).values({
          ...tenancy(opForeign), plate: 'FG-01', active: true,
        }).returning();
  ...
  describe('vehicles(op)', () => {
    it('returns paired vehicles and excludes an unpaired vehicle seeded alongside', async () => {
      const captured = await withTxIsolation(testDb, async (tx) => {
        ...
        const [drvP] = await tx.insert(driver).values({
          ...tenancy(op), fullName: 'VPairedDrv', active: true, operatorId: opP,
        }).returning();
        const [vehP] = await tx.insert(vehicle).values({
          ...tenancy(op), plate: 'AAA-VP-01', active: true,
        }).returning();
        await tx.insert(vehicle).values({
          ...tenancy(op), plate: 'BBB-VU-01', active: true,
        });
        if (!drvP || !vehP) throw new Error('seed failed');
        await tx.insert(driverVehicleAssignment).values({
          ...tenancy(op), driverId: drvP.driverId, vehicleId: vehP.vehicleId,
    ...
    it('excludes a vehicle whose only assignment is revoked while keeping a sibling paired vehicle', async () => {
      const captured = await withTxIsolation(testDb, async (tx) => {
        ...
        const [drvOk] = await tx.insert(driver).values({
          ...tenancy(op), fullName: 'VOkDrv', active: true, operatorId: opOk,
        }).returning();
        const [drvRev] = await tx.insert(driver).values({
          ...tenancy(op), fullName: 'VRevDrv', active: true, operatorId: opRev,
        }).returning();
        const [vehOk] = await tx.insert(vehicle).values({
          ...tenancy(op), plate: 'AAA-VOK-01', active: true,
        }).returning();
        const [vehRev] = await tx.insert(vehicle).values({
          ...tenancy(op), plate: 'BBB-VRV-01', active: true,
        }).returning();
        if (!drvOk || !drvRev || !vehOk || !vehRev) throw new Error('seed failed');
        await tx.insert(driverVehicleAssignment).values([
          { ...tenancy(op), driverId: drvOk.driverId, vehicleId: vehOk.vehicleId },
    ...
    it('excludes a vehicle paired only with an inactive driver while keeping a sibling paired vehicle', async () => {
      const captured = await withTxIsolation(testDb, async (tx) => {
        ...
        const [drvOk] = await tx.insert(driver).values({
          ...tenancy(op), fullName: 'V2OkDrv', active: true, operatorId: opOk,
        }).returning();
        const [drvIA] = await tx.insert(driver).values({
          ...tenancy(op), fullName: 'V2InactDrv', active: false, operatorId: opID,
        }).returning();
        const [vehOk] = await tx.insert(vehicle).values({
          ...tenancy(op), plate: 'AAA-VOK-02', active: true,
        }).returning();
        const [vehID] = await tx.insert(vehicle).values({
          ...tenancy(op), plate: 'BBB-VID-01', active: true,
        }).returning();
    ...
    it('excludes pairings whose joined driver belongs to another company while keeping in-company pair', async () => {
      const captured = await withTxIsolation(testDb, async (tx) => {
        ...
        const [drvOk] = await tx.insert(driver).values({
          ...tenancy(opMine), fullName: 'V3OkDrv', active: true, operatorId: opIdOk,
        }).returning();
        const [drvForeign] = await tx.insert(driver).values({
          ...tenancy(opForeign), fullName: 'V3ForDrv', active: true, operatorId: opIdForeign,
        }).returning();
        const [vehOk] = await tx.insert(vehicle).values({
          ...tenancy(opMine), plate: 'AAA-VOK-03', active: true,
        }).returning();
        const [vehBad] = await tx.insert(vehicle).values({
          ...tenancy(opMine), plate: 'BBB-VBD-01', active: true,
        }).returning();
