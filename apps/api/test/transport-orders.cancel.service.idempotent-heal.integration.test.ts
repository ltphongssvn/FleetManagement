...
async function seedActivePair(tx: TestTx, op: ReturnType<typeof createOperatorContext>): Promise<{ operatorId: string; vehicleId: string }> {
  ...
  const [d] = await tx.insert(driver).values({ ...tn, fullName: 'HealDriver', operatorId: op.operatorId }).returning({ driverId: driver.driverId });
  const [v] = await tx.insert(vehicle).values({ ...tn, plate: 'HEAL-' + randomUUID().slice(0, 4) }).returning({ vehicleId: vehicle.vehicleId });
...
describe('@fleet/api - TransportOrdersCancelService idempotent heals stale road_runs', () => {
  ...
  it('on idempotent re-cancel, ensures linked road_runs are also cancelled even if a prior partial cancel left them stale', async () => {
    ...
    await withTxIsolation(testDb, async (tx) => {
      ...
      const [before] = await tx.select({ state: roadRun.state }).from(roadRun)
      ...
      const r = await cancelSvc.cancel(created.transportOrderId, { reason: 'customer_request' }, op);
