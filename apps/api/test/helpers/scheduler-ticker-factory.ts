...
export function monitorTicker(
  ...
  switch (kind) {
    case 'intakeLag':
      return { key: 'intakeLag', tag: 'intake-lag-check', label: 'Intake-lag check failed: ', intervalMs: INTERVALS.intakeLag, run };
    case 'alertLag':
      return { key: 'alertLag', tag: 'driver-alert-lag-check', label: 'Driver-alert-lag check failed: ', intervalMs: INTERVALS.alertLag, run };
...
export function coreTickers(deps: {
  ...
  return [
    { key: 'outbox', tag: 'outbox-drain', label: 'Outbox drain failed: ', intervalMs: INTERVALS.outbox, run: deps.outbox },
    { key: 'projection', tag: 'projection-drain', label: 'Projection drain failed: ', intervalMs: INTERVALS.projection, run: deps.projection },
    { key: 'reconciler', tag: 'commands-reconciler', label: 'Reconciler tick failed: ', intervalMs: INTERVALS.reconciler, run: deps.reconciler },
  ];
}
