...
async function seedFleet(
  ...
  const tn = tenancy(op);
  const [vGhost] = await tx
    .insert(vehicle)
    .values({ ...tn, plate: 'RG-GHOST-01', active: true })
    .returning();
  const [vOrphan] = await tx
    .insert(vehicle)
    .values({ ...tn, plate: 'RG-ORPH-01', active: true })
    .returning();
  const [vLive] = await tx
    .insert(vehicle)
    .values({ ...tn, plate: 'RG-LIVE-01', active: true })
    .returning();
  const [vDone] = await tx
    .insert(vehicle)
    .values({ ...tn, plate: 'RG-DONE-01', active: true })
    .returning();
  ...
  const [rrGhost] = await tx
    ...
    .values({
      ...tn,
      state: 'started',
      assignedOperatorId: '00000000-0000-0000-0000-000000020001',
      assignedAssetId: vGhost.vehicleId,
    })
    .returning();
  const [oCancelled] = await tx
  ...
  await tx.insert(roadRunTransportOrder).values({
    ...
    transportOrderId: oCancelled.transportOrderId,
    sequence: 1,
  });
  // ORPHAN: planned run, zero links (Jul-05 teardown class).
  const [rrOrphan] = await tx
    .insert(roadRun)
    .values({
      ...tn,
      state: 'planned',
      assignedOperatorId: '00000000-0000-0000-0000-000000020002',
      assignedAssetId: vOrphan.vehicleId,
    })
    .returning();
  ...
  const [rrLive] = await tx
    ...
    .values({
      ...tn,
      state: 'started',
      assignedOperatorId: '00000000-0000-0000-0000-000000020003',
      assignedAssetId: vLive.vehicleId,
    })
    .returning();
  const [oLive] = await tx
    .insert(transportOrder)
    .values({ ...tn })
    .returning();
  ...
  await tx.insert(roadRunTransportOrder).values({
    ...
    transportOrderId: oLive.transportOrderId,
    sequence: 1,
  });
  // DONE control: already-cancelled run with cancelled order -- not a ghost.
  const [rrDone] = await tx
    .insert(roadRun)
    .values({
      ...tn,
      state: 'cancelled',
      assignedOperatorId: '00000000-0000-0000-0000-000000020004',
      assignedAssetId: vDone.vehicleId,
    })
    .returning();
  const [oDone] = await tx
    .insert(transportOrder)
    .values({
      ...tn,
      state: 'cancelled',
  ...
  await tx.insert(roadRunTransportOrder).values({
    ...tn,
    roadRunId: rrDone.roadRunId,
    transportOrderId: oDone.transportOrderId,
    sequence: 1,
  });
  return {
    ghostRunId: rrGhost.roadRunId,
    orphanRunId: rrOrphan.roadRunId,
    liveRunId: rrLive.roadRunId,
    doneRunId: rrDone.roadRunId,
  };
}
describe('@fleet/api - repairGhostRuns (compensating road_run.cancelled events)', () => {
  ...
  it('execute cancels ghosts, appends road_run.cancelled feed + outbox rows, leaves live and done untouched, and is idempotent', async () => {
    ...
    const cancelledFor = (id: string): boolean =>
