...
async function seedPair(
...
): Promise<{ operatorId: string; vehicleId: string; driverFullName: string }> {
  const tn = {
  ...
  const [d] = await tx
    .insert(driver)
    .values({ ...tn, fullName: driverFullName, operatorId: op.operatorId })
    .returning({ driverId: driver.driverId });
  const [v] = await tx
    .insert(vehicle)
    .values({ ...tn, plate: 'CT-' + randomUUID().slice(0, 4) })
...
describe('@fleet/api - cargo/customer/driver enrichment on review query (T7)', () => {
  ...
  it('persists cargoTypeId and returns cargoName on the review row', async () => {
    ...
    await withTxIsolation(testDb, async (tx) => {
      ...
      const [cg] = await tx
        .insert(cargoType)
        .values({ ...tn, name: 'GẠO-T7' })
        .returning({ cargoTypeId: cargoType.cargoTypeId });
      ...
      const created = await svc.create(
        {
          ...
          roadRun: {
            plannedStartAt: '2026-06-01T07:00:00.000Z',
            assignedOperatorId: seeded.operatorId,
            assignedAssetId: seeded.vehicleId,
          },
        },
        op,
      );
