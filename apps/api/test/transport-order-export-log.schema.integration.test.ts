...
describe('@fleet/api - transport_order_export_log schema (integration)', () => {
  ...
  it('table exists with the required audit columns', async () => {
    const r = await testDb.db.execute<{ column_name: string }>(sql.raw(
      'SELECT column_name FROM information_schema.columns WHERE table_name = ' + "'transport_order_export_log'"
    ));
    const cols = r.rows.map((x) => x.column_name);
    expect(cols).toEqual(expect.arrayContaining([
      'export_log_id', 'company_id', 'operator_id', 'trigger',
      'day_key', 'row_count', 'sha256', 'filename', 'created_at',
    ]));
  });
  ...
  it('CHECK constraint rejects trigger not in (manual, login, logout)', async () => {
    const q =
      ...
      ' operator_id, trigger, day_key, row_count, sha256, filename) ' +
      "VALUES ('" + CO + "','" + CO + "','" + CO + "','" + CO + "', " +
      "'" + OP + "','bogus','2026-05-26',5,'h','f.xlsx')";
