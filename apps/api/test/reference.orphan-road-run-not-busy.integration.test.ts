...
async function seedOrphanAndLinkedBusy(
  ...
  const [drvOrphan] = await tx
    .insert(driver)
    .values({
      ...tenancy(op),
      fullName: 'AAA OrphanDrv',
      active: true,
      operatorId: ids.orphanOp,
    })
    .returning();
  const [drvLinked] = await tx
    .insert(driver)
    .values({
      ...tenancy(op),
      fullName: 'BBB LinkedBusyDrv',
      active: true,
      operatorId: ids.linkedOp,
    })
    .returning();
  const [vehOrphan] = await tx
    .insert(vehicle)
    .values({
      ...tenancy(op),
      plate: ids.orphanPlate,
      active: true,
    })
    .returning();
  const [vehLinked] = await tx
    .insert(vehicle)
    .values({
      ...tenancy(op),
      plate: ids.linkedPlate,
      active: true,
    })
    .returning();
  ...
  await tx.insert(roadRun).values({
    ...tenancy(op),
    state: 'planned',
    assignedOperatorId: ids.orphanOp,
    assignedAssetId: vehOrphan.vehicleId,
  });
  // LINKED BUSY control: non-terminal road_run WITH a linked order.
  const [rrLinked] = await tx
    .insert(roadRun)
    .values({
      ...tenancy(op),
      state: 'planned',
      assignedOperatorId: ids.linkedOp,
      assignedAssetId: vehLinked.vehicleId,
    })
    .returning();
  ...
  it('drivers(op) includes the orphan-bound driver and still hides the linked-busy driver', async () => {
    const result = await withTxIsolation(testDb, async (tx) => {
      ...
      const seeded = await seedOrphanAndLinkedBusy(tx, op, {
        ...
        orphanPlate: 'ORPH-D-01',
        linkedPlate: 'LINK-D-01',
      });
  ...
  it('vehicles(op) includes the orphan-bound vehicle and still hides the linked-busy vehicle', async () => {
    const result = await withTxIsolation(testDb, async (tx) => {
      ...
      const seeded = await seedOrphanAndLinkedBusy(tx, op, {
        ...
        orphanPlate: 'AAA-ORPH-V-01',
        linkedPlate: 'BBB-LINK-V-01',
      });
      return { items: (await svc.vehicles(op)).items, seeded };
    });
    expect(result?.items).toEqual([{ id: result?.seeded.orphanVehicleId, label: 'AAA-ORPH-V-01' }]);
  });
  it('driverVehicleAssignments(op) includes the orphan-bound pair and still hides the linked-busy pair', async () => {
    const result = await withTxIsolation(testDb, async (tx) => {
      ...
      const seeded = await seedOrphanAndLinkedBusy(tx, op, {
        ...
        orphanPlate: 'ORPH-A-01',
        linkedPlate: 'LINK-A-01',
      });
