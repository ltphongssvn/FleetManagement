...
async function seedPair(
  tx: TestTx,
  suffix: string,
): Promise<{ driverId: string; vehicleId: string; assignmentId: string }> {
  const tn = tenancy();
  const operatorId = randomUUID();
  const [d] = await tx
    .insert(driver)
    .values({ ...tn, fullName: 'DEL-' + suffix, operatorId })
    .returning({ driverId: driver.driverId });
  const [v] = await tx
    .insert(vehicle)
    .values({ ...tn, plate: 'DEL-' + suffix })
...
describe('@fleet/api - soft-delete cascades revocation onto active driver_vehicle_assignment', () => {
  ...
  it('deleteVehicle() revokes the active assignment referencing that vehicle', async () => {
    const captured = await withTxIsolation(testDb, async (tx) => {
      ...
      const [row] = await tx
        .select({ revokedAt: driverVehicleAssignment.revokedAt })
        .from(driverVehicleAssignment)
        .where(
          and(
            eq(driverVehicleAssignment.companyId, OP.companyId),
            eq(driverVehicleAssignment.assignmentId, pair.assignmentId),
          ),
        );
