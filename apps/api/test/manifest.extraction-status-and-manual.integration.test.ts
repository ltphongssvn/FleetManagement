...
async function verifyingManifest(): Promise<string> {
  ...
  const c = await service.commitUpload(
    { uploadSessionId: negotiated.uploadSessionId, actualSizeBytes: 900 },
    OP,
  );
  return c.manifestId;
...
async function statusOf(manifestId: string): Promise<string> {
  const r = await testDb.db.execute(
    sql`SELECT extraction_status FROM manifest WHERE manifest_id = ${manifestId}::uuid`,
  );
...
describe('@fleet/api - manifest extraction_status persistence + manual edit', () => {
  beforeAll(async () => {
    testDb = await startMigratedTestDb('fleet_test_extstatus');
  });
  afterAll(async () => {
  ...
  it('finalizeExtraction(not_found) persists status=not_found (kg stays null)', async () => {
    const manifestId = await committedManifest();
    await service.finalizeExtraction(
      { manifestId, status: 'not_found', extractedNetWeightKg: null },
      OP,
    );
  ...
  it('finalizeExtraction(unreadable) persists status=unreadable (kg stays null)', async () => {
    const manifestId = await committedManifest();
    await service.finalizeExtraction(
      { manifestId, status: 'unreadable', extractedNetWeightKg: null },
      OP,
    );
