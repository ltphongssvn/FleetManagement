...
async function committedManifest(): Promise<string> {
  ...
  const fin = await service.finalizeIntake(
...
describe('@fleet/api - finalizeExtraction persists kg + reason + emits projection event', () => {
  beforeAll(async () => {
    testDb = await startMigratedTestDb('fleet_test_extres');
  });
  afterAll(async () => {
  ...
  it('extracted: writes kg, no reason, emits manifest.net_weight_extracted to projections', async () => {
    ...
    const r = await service.finalizeExtraction(
      { manifestId, status: 'extracted', extractedNetWeightKg: 20730 },
      OP,
    );
    expect(r).toMatchObject({ manifestId, status: 'extracted' });
    const row = await testDb.db.execute(
      sql`SELECT extracted_net_weight_kg, extraction_status, extraction_reason FROM manifest WHERE manifest_id = ${manifestId}::uuid`,
    );
    expect(row.rows[0]).toMatchObject({
      extracted_net_weight_kg: '20730.000',
  ...
  it('not_found: kg null, persists reason=object_missing, no projection event', async () => {
    const manifestId = await committedManifest();
    await service.finalizeExtraction(
      { manifestId, status: 'not_found', extractedNetWeightKg: null, reason: 'object_missing' },
      OP,
    );
    const row = await testDb.db.execute(
      sql`SELECT extracted_net_weight_kg, extraction_status, extraction_reason FROM manifest WHERE manifest_id = ${manifestId}::uuid`,
    ...
    expect(row.rows[0]).toMatchObject({
      extracted_net_weight_kg: null,
      extraction_status: 'not_found',
      extraction_reason: 'object_missing',
    });
    const ob = await testDb.db.execute(sql`
  ...
  it('unreadable: kg null, persists reason=unparseable, no projection event (guard arm)', async () => {
    const manifestId = await committedManifest();
    const r = await service.finalizeExtraction(
      { manifestId, status: 'unreadable', extractedNetWeightKg: null, reason: 'unparseable' },
      OP,
    );
  ...
  it('direct service call with extracted+null kg is a no-op (controller schema cannot reach this; service must still guard)', async () => {
    const manifestId = await committedManifest();
    const r = await service.finalizeExtraction(
      { manifestId, status: 'extracted', extractedNetWeightKg: null },
      OP,
    );
    expect(r).toMatchObject({ manifestId, status: 'extracted' });
    const row = await testDb.db.execute(
      sql`SELECT extracted_net_weight_kg FROM manifest WHERE manifest_id = ${manifestId}::uuid`,
    );
    expect(row.rows[0]).toMatchObject({ extracted_net_weight_kg: null });
    const ob = await testDb.db.execute(
      sql`SELECT 1 FROM outbox WHERE payload->>'eventType' = 'manifest.net_weight_extracted'`,
    );
    expect(ob.rows).toHaveLength(0);
  });
  ...
  it('extracted + non-null kg on a non-existent manifest throws invalid-transition (no committed row to update)', async () => {
    ...
    await expect(
      service.finalizeExtraction(
        { manifestId: missingManifestId, status: 'extracted', extractedNetWeightKg: 20730 },
        OP,
      ),
