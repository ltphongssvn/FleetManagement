...
describe('@fleet/api - ErpInboundController', () => {
  ...
  beforeEach(() => {
    ...
    ctl = new ErpInboundController(svc);
    process.env['ERP_WEBHOOK_SECRET'] = 'test-secret'; // pragma: allowlist secret
  });
  ...
  it('accepts valid signature and delegates', async () => {
    ...
    const sig = createHmac('sha256', 'test-secret').update(JSON.stringify(body)).digest('hex');
