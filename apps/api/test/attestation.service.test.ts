...
import {
  AttestationService,
  type AttestationServiceDeps,
} from '../src/device/attestation.service.js';
...
describe('AttestationService (hardware attestation dispatch)', () => {
  ...
  it('passes the expected challenge and injected trust-root into the Android verifier', async () => {
    ...
    const arg = verifyAndroid.mock.calls[0]?.[1] as {
      expectedChallenge: string;
      isTrustedRoot: unknown;
    };
  ...
  it('dispatches iOS tokens to the App Attest verifier with keyId + derived clientDataHash', async () => {
    const verifyIos = vi.fn().mockResolvedValue({
      kind: 'ok',
      environment: 'production',
      publicKeySpkiBase64: 'CCCC',
    } satisfies IosAppAttestOutcome);
    const svc = new AttestationService(makeDeps({ verifyIos }));
    const r = await svc.verify({
      platform: 'ios',
      token: IOS_OBJECT_B64,
      expectedNonce: 'nonce-2',
      keyId: KEY_ID_B64,
    });
    expect(r.kind).toBe('ok');
    expect(verifyIos).toHaveBeenCalledOnce();
    const arg = verifyIos.mock.calls[0]?.[1] as {
  ...
  it('maps Android untrusted-root to device-untrusted', async () => {
    const verifyAndroid = vi
      .fn()
      .mockResolvedValue({ kind: 'untrusted-root' } satisfies AndroidKeyAttestationOutcome);
    const svc = new AttestationService(makeDeps({ verifyAndroid }));
    expect(
      (await svc.verify({ platform: 'android', token: ANDROID_CHAIN_B64, expectedNonce: 'n' }))
        .kind,
    ).toBe('device-untrusted');
  });

  it('maps Android weak-security-level to device-untrusted', async () => {
    const verifyAndroid = vi
      .fn()
      .mockResolvedValue({ kind: 'weak-security-level' } satisfies AndroidKeyAttestationOutcome);
    const svc = new AttestationService(makeDeps({ verifyAndroid }));
    expect(
      (await svc.verify({ platform: 'android', token: ANDROID_CHAIN_B64, expectedNonce: 'n' }))
        .kind,
  ...
  it('maps Android challenge-mismatch to nonce-mismatch', async () => {
    const verifyAndroid = vi
      .fn()
      .mockResolvedValue({ kind: 'challenge-mismatch' } satisfies AndroidKeyAttestationOutcome);
    const svc = new AttestationService(makeDeps({ verifyAndroid }));
    expect(
      (await svc.verify({ platform: 'android', token: ANDROID_CHAIN_B64, expectedNonce: 'n' }))
        .kind,
  ...
  it('maps Android empty-chain / parse failure to invalid-platform-data', async () => {
    const verifyAndroid = vi.fn().mockResolvedValue({
      kind: 'certificate-parse-failed',
    } satisfies AndroidKeyAttestationOutcome);
    const svc = new AttestationService(makeDeps({ verifyAndroid }));
    expect(
      (await svc.verify({ platform: 'android', token: ANDROID_CHAIN_B64, expectedNonce: 'n' }))
        .kind,
    ).toBe('invalid-platform-data');
  });

  it('maps iOS nonce-mismatch to nonce-mismatch', async () => {
    const verifyIos = vi
      .fn()
      .mockResolvedValue({ kind: 'nonce-mismatch' } satisfies IosAppAttestOutcome);
    const svc = new AttestationService(makeDeps({ verifyIos }));
    expect(
      (
        await svc.verify({
  ...
  it('maps iOS rp-id-mismatch to app-untrusted', async () => {
    const verifyIos = vi
      .fn()
      .mockResolvedValue({ kind: 'rp-id-mismatch' } satisfies IosAppAttestOutcome);
    const svc = new AttestationService(makeDeps({ verifyIos }));
    expect(
      (
        await svc.verify({
  ...
  it('maps iOS untrusted-root to device-untrusted', async () => {
    const verifyIos = vi
      .fn()
      .mockResolvedValue({ kind: 'untrusted-root' } satisfies IosAppAttestOutcome);
    const svc = new AttestationService(makeDeps({ verifyIos }));
    expect(
      (
        await svc.verify({
  ...
  it('maps iOS malformed-object to invalid-platform-data', async () => {
    const verifyIos = vi
      .fn()
      .mockResolvedValue({ kind: 'malformed-object' } satisfies IosAppAttestOutcome);
    const svc = new AttestationService(makeDeps({ verifyIos }));
    expect(
      (
        await svc.verify({
  ...
  it('returns invalid-platform-data for an iOS request missing keyId', async () => {
    ...
    const svc = new AttestationService(makeDeps({ verifyIos }));
    expect(
      (await svc.verify({ platform: 'ios', token: IOS_OBJECT_B64, expectedNonce: 'n' })).kind,
    ).toBe('invalid-platform-data');
  ...
  it('returns invalid-platform-data for an iOS token that decodes to empty bytes', async () => {
    ...
    const svc = new AttestationService(makeDeps({ verifyIos }));
    expect(
      (await svc.verify({ platform: 'ios', token: ' ', expectedNonce: 'n', keyId: KEY_ID_B64 }))
        .kind,
    ).toBe('invalid-platform-data');
    expect(verifyIos).not.toHaveBeenCalled();
  });
  ...
  it('returns invalid-platform-data when the token is not valid base64 bytes', async () => {
    ...
    expect((await svc.verify({ platform: 'android', token: '', expectedNonce: 'n' })).kind).toBe(
