...
describe('AdminDriversCreateController', () => {
  ...
  beforeEach(() => {
    createFn = vi.fn();
    controller = new AdminDriversCreateController({
      create: createFn,
    } as unknown as AdminDriversCreateService);
  });
  ...
  it('rejects invalid body (missing fullName)', async () => {
    await expect(
      controller.create(op, { phone: '+84901000001', password: 'p' } as never),
    ).rejects.toThrow();
  });

  it('rejects invalid body (password too short)', async () => {
    await expect(
      controller.create(op, { fullName: 'A', phone: '+84901000001', password: 'x' }),
    ).rejects.toThrow();
  });

  it('rejects invalid body (phone too short)', async () => {
    await expect(
      controller.create(op, { fullName: 'A', phone: '1', password: 'secret123' }),
    ).rejects.toThrow(); // pragma: allowlist secret
  });
});
