...
interface FakeDbResult {
  db: object;
  updates: { outboxId: string; status?: string; attempts?: number; nextAttemptAt?: Date | null }[];
}
...
vi.mock('drizzle-orm', () => {
  ...
  const sql = Object.assign((..._args: unknown[]) => ({ _sql: true }), {
...
describe('OutboxRelayService - drainOnce', () => {
  ...
  it('dead-letters rows with unknown aggregate', async () => {
    ...
    const u = updates[0];
    if (!u) throw new Error('expected update');
  ...
  it('dead-letters when attempts reach max threshold', async () => {
    ...
    const u = updates[0];
    if (!u) throw new Error('expected update');
    expect(u).toMatchObject({ status: 'dead_letter', attempts: 5 });
  });
    ...
    const rows: FakeRow[] = [
      {
        ...
        attempts: 0,
        nextAttemptAt: null,
        payload: { eventType: 'road_run.a', aggregateType: 'road_run' },
      },
      {
        outboxId: 'r8',
        queueName: 'projections',
        ...
        attempts: 0,
        nextAttemptAt: null,
        payload: { eventType: 'road_run.b', aggregateType: 'road_run' },
      },
    ];
    const { db } = makeFakeDb(rows);
    const svc = new OutboxRelayService(db as never, { url: 'redis://x' } as never);
    ...
    const rows: FakeRow[] = [
      {
        ...
        attempts: 0,
        nextAttemptAt: null,
        payload: { not_a_known_shape: true } as unknown as {
          eventType: string;
  ...
  it('logs non-Error thrown values via String(err) (line 153 branch)', async () => {
    ...
    expect(result.retryScheduled).toBe(1);
    const u0 = updates[0];
    if (u0 === undefined) throw new Error('expected update');
    expect(u0).toMatchObject({ attempts: 1 });
  });
});
