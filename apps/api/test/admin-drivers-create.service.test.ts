...
function makeDb(): { db: unknown; inserts: DriverInsertCapture[] } {
  ...
  const db: Record<string, unknown> = {
    ...
    select: () => ({
      from: () => ({ where: () => ({ limit: (): Promise<unknown[]> => Promise.resolve([]) }) }),
    }),
    insert: (): {
      values: (v: DriverInsertCapture) => { returning: () => Promise<DriverInsertCapture[]> };
    } => ({
      values: (v: DriverInsertCapture) => ({
        returning: (): Promise<DriverInsertCapture[]> => {
          ...
          return Promise.resolve([
            {
              ...row,
              ...({ driverId: 'drv-' + String(inserts.length) } as Partial<DriverInsertCapture>),
            },
          ]);
        },
      }),
...
describe('AdminDriversCreateService', () => {
  ...
  it('throws when the DB returns no row (line 49 branch)', async () => {
    ...
    await expect(
      svc.create({ fullName: 'A', phone: '+84901000099', password: 'pw', ...tenancy }),
    ).rejects.toThrow(/Driver insert failed/); // pragma: allowlist secret
  });
});
