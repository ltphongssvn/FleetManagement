...
function q(s: string): string {
  return String.fromCharCode(39) + s + String.fromCharCode(39);
}
...
async function seedOrderGraph(opts: {
  ...
  stmts.push(
    ...
    'INSERT INTO road_run_transport_order (road_run_transport_order_id, company_id, business_unit_id, depot_id, legal_entity_id, road_run_id, transport_order_id, sequence) VALUES (' +
      q('00000000-0000-4000-8000-000000040001') +
      ',' +
      q(co) +
      ',' +
      q(co) +
  ...
  stmts.push(
    'INSERT INTO stop (stop_id, company_id, business_unit_id, depot_id, legal_entity_id, transport_order_id, sequence, stop_type, yard_id, arrived_at) VALUES (' +
      q('00000000-0000-4000-8000-000000050001') +
      ',' +
      q(co) +
      ',' +
      q(co) +
      ...
      ')',
    'INSERT INTO stop (stop_id, company_id, business_unit_id, depot_id, legal_entity_id, transport_order_id, sequence, stop_type, yard_id, arrived_at) VALUES (' +
      q('00000000-0000-4000-8000-000000050002') +
      ',' +
      q(co) +
      ',' +
      q(co) +
...
describe('@fleet/api - TransportOrdersExportService (integration)', () => {
  ...
  it('manual: inserts a ledger row with stable sha256 + canonical filename', async () => {
    ...
    const logRows = await testDb.db.execute(
      sql.raw('SELECT trigger, row_count, sha256 FROM transport_order_export_log'),
    );
  ...
  it('login: idempotent — second call same day returns the same ledger id', async () => {
    ...
    const cnt = await testDb.db.execute<{ c: number }>(
      sql.raw("SELECT COUNT(*)::int AS c FROM transport_order_export_log WHERE trigger = 'login'"),
    );
  ...
  it('logout: idempotent same day', async () => {
    ...
    await svc.exportAndLog(OP, 'logout');
    const cnt = await testDb.db.execute<{ c: number }>(
      sql.raw("SELECT COUNT(*)::int AS c FROM transport_order_export_log WHERE trigger = 'logout'"),
    );
    expect((cnt.rows[0] as { c: number }).c).toBe(1);
  });
  ...
  it('tenant scope: rows from another company are not exported', async () => {
    ...
    const stmt =
      ...
      ' state, stop_count, transport_order_refs, server_seq, planned_start_at) ' +
      'VALUES (' +
      q('ffffffff-1111-4111-8111-111111111111') +
      ',' +
      q(otherCo) +
      ',' +
      q(otherCo) +
    ...
    await testDb.db.execute(
      sql.raw(
        'INSERT INTO manifest (manifest_id, company_id, business_unit_id, depot_id, legal_entity_id, transport_order_id, manifest_correlation_id, stop_id, state, extracted_net_weight_kg, extraction_status) VALUES (' +
          ...
          q(toId) +
          ',' +
          q('00000000-0000-7000-8000-000000770001') +
          ',' +
          q(pickupStopId) +
          ',' +
          q('committed') +
    ...
    await testDb.db.execute(
      sql.raw(
        'INSERT INTO upload_session (upload_session_id, company_id, business_unit_id, depot_id, legal_entity_id, manifest_id, operator_id, s3_key, s3_bucket, content_type, state) VALUES (' +
          ...
          q(manifestId) +
          ',' +
          q('00000000-0000-4000-8000-0000000b0001') +
          ',' +
          q('manifests/x/y/z.jpg') +
          ',' +
  ...
  it('emits a Chênh lệch column with the numeric delivery-minus-pickup weight diff', async () => {
    ...
    await testDb.db.execute(
      sql.raw(
        'INSERT INTO manifest (manifest_id, company_id, business_unit_id, depot_id, legal_entity_id, transport_order_id, manifest_correlation_id, stop_id, state, extracted_net_weight_kg, extraction_status) VALUES (' +
          q('00000000-0000-4000-8000-0000000d8001') +
          ',' +
          q(co) +
          ',' +
          q(co) +
          ...
          q(toId) +
          ',' +
          q('00000000-0000-7000-8000-000000780001') +
          ',' +
          q(pickupStopId) +
          ',' +
          q('committed') +
    ...
    await testDb.db.execute(
      sql.raw(
        'INSERT INTO upload_session (upload_session_id, company_id, business_unit_id, depot_id, legal_entity_id, manifest_id, operator_id, s3_key, s3_bucket, content_type, state) VALUES (' +
          q('00000000-0000-4000-8000-0000000e8001') +
          ',' +
          q(co) +
          ',' +
          q(co) +
          ...
          q('00000000-0000-4000-8000-0000000d8001') +
          ',' +
          q('00000000-0000-4000-8000-0000000b8001') +
          ',' +
          q('manifests/x/y/p.jpg') +
          ',' +
          q('fleet-pilot-artifacts') +
          ',' +
          q('image/jpeg') +
    ...
    await testDb.db.execute(
      sql.raw(
        'INSERT INTO manifest (manifest_id, company_id, business_unit_id, depot_id, legal_entity_id, transport_order_id, manifest_correlation_id, stop_id, state, extracted_net_weight_kg, extraction_status) VALUES (' +
          q('00000000-0000-4000-8000-0000000d8002') +
          ',' +
          q(co) +
          ',' +
          q(co) +
          ...
          q(toId) +
          ',' +
          q('00000000-0000-7000-8000-000000780002') +
          ',' +
          q(deliveryStopId) +
          ',' +
          q('committed') +
    ...
    await testDb.db.execute(
      sql.raw(
        'INSERT INTO upload_session (upload_session_id, company_id, business_unit_id, depot_id, legal_entity_id, manifest_id, operator_id, s3_key, s3_bucket, content_type, state) VALUES (' +
          q('00000000-0000-4000-8000-0000000e8002') +
          ',' +
          q(co) +
          ',' +
          q(co) +
          ...
          q('00000000-0000-4000-8000-0000000d8002') +
          ',' +
          q('00000000-0000-4000-8000-0000000b8002') +
          ',' +
          q('manifests/x/y/d.jpg') +
          ',' +
          q('fleet-pilot-artifacts') +
          ',' +
          q('image/jpeg') +
