...
describe('@fleet/api - assertSingleInstance', () => {
  ...
  it('is a no-op when EXPECTED_INSTANCE_COUNT=1', () => {
    expect(() => {
      assertSingleInstance({ EXPECTED_INSTANCE_COUNT: '1' });
    }).not.toThrow();
  });

  it('throws when EXPECTED_INSTANCE_COUNT>1 (pilot invariant: single-node Socket.IO per PDF Day-One §6)', () => {
    expect(() => {
      assertSingleInstance({ EXPECTED_INSTANCE_COUNT: '2' });
    }).toThrow(/single-instance invariant violated/i);
  });

  it('throws when EXPECTED_INSTANCE_COUNT is non-numeric (fail-fast on misconfiguration)', () => {
    expect(() => {
      assertSingleInstance({ EXPECTED_INSTANCE_COUNT: 'abc' });
    }).toThrow(/EXPECTED_INSTANCE_COUNT must be a positive integer/i);
  });
