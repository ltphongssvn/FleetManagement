...
import { findDeliveredIncompleteRuns, repairCompleteDeliveredRuns } from '../src/maintenance/repair-complete-delivered-runs.js';
...
async function seedFleet(tx: TestTx, op: ReturnType<typeof createOperatorContext>): Promise<Seeded> {
  const deliveredRunId = await seedRun(tx, op, "RC-DELIV-01", "00000000-0000-0000-0000-000000030001", "started", 2, 2);
  const incompleteRunId = await seedRun(tx, op, "RC-INCMP-01", "00000000-0000-0000-0000-000000030002", "started", 2, 1);
  const doneRunId = await seedRun(tx, op, "RC-DONE-01", "00000000-0000-0000-0000-000000030003", "completed", 2, 2);
...
describe("@fleet/api - repairCompleteDeliveredRuns (compensating road_run.completed events)", () => {
  ...
  it("execute completes delivered runs, appends road_run.completed feed + outbox rows, leaves incomplete + done untouched, and is idempotent", async () => {
    ...
    const completedFor = (id: string): boolean => (result?.feedRows ?? []).some((e) => {
  ...
  it('excludes an orphan non-terminal run with zero linked orders (runIsDelivered false branch)', async () => {
    const result = await withTxIsolation(testDb, async (tx) => {
      const op = createOperatorContext();
      const tn = tenancy(op);
      const [v] = await tx.insert(vehicle).values({ ...tn, plate: 'RC-ORPH-01', active: true }).returning();
      if (!v) throw new Error('vehicle seed failed');
      const [rr] = await tx.insert(roadRun).values({
        ...tn, state: 'started',
        assignedOperatorId: '00000000-0000-0000-0000-000000030009', assignedAssetId: v.vehicleId,
        startedAt: new Date(),
      }).returning();
  ...
  it('execute with no delivered runs returns found:0 repaired:0 and mutates nothing (empty-set branch)', async () => {
    const result = await withTxIsolation(testDb, async (tx) => {
      const op = createOperatorContext();
      const tn = tenancy(op);
      const [v] = await tx.insert(vehicle).values({ ...tn, plate: 'RC-NONE-01', active: true }).returning();
      if (!v) throw new Error('vehicle seed failed');
      const [rr] = await tx.insert(roadRun).values({
        ...tn, state: 'started',
        assignedOperatorId: '00000000-0000-0000-0000-000000030010', assignedAssetId: v.vehicleId,
        startedAt: new Date(),
      }).returning();
      const [o] = await tx.insert(transportOrder).values({ ...tn, state: 'draft' }).returning();
