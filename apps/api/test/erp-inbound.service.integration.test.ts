...
describe('@fleet/api - ErpInboundService (integration)', () => {
  beforeAll(async () => {
    testDb = await startMigratedTestDb('fleet_erp_inbound');
  });
  afterAll(async () => {
  ...
  it('updates row to acknowledged with timestamp', async () => {
    await withTxIsolation(testDb as never, async (tx) => {
      ...
      const sel =
        'SELECT status, external_erp_invoice_id FROM erp_invoice_map WHERE manifest_correlation_id = ' +
        String.fromCharCode(39) +
        MCID +
        String.fromCharCode(39);
      const rows = await tx.execute<{ status: string; external_erp_invoice_id: string }>(
        sql.raw(sel),
      );
  ...
  it('updates row to failed with failureReason', async () => {
    await withTxIsolation(testDb as never, async (tx) => {
      ...
      expect(res.updated).toBe(true);
      const sel =
        'SELECT status, failure_reason FROM erp_invoice_map WHERE manifest_correlation_id = ' +
        String.fromCharCode(39) +
        MCID +
        String.fromCharCode(39);
