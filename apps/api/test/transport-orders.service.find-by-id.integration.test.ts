...
async function seedActivePair(
  ...
  const [d] = await tx
    .insert(driver)
    .values({ ...tn, fullName: 'FB', operatorId })
    .returning({ driverId: driver.driverId });
  const [v] = await tx
    .insert(vehicle)
    .values({ ...tn, plate: 'FB-' + randomUUID().slice(0, 4) })
...
describe('@fleet/api - TransportOrdersService.findById (integration)', () => {
  ...
  it('returns the enriched row for an order in the calling tenancy', async () => {
    ...
    await withTxIsolation(testDb, async (tx) => {
      ...
      try {
        row = await svc.findById(created.transportOrderId, op);
      } catch (e) {
        thrown = e;
      }
    });
    ...
    await withTxIsolation(testDb, async (tx) => {
      ...
      await tx.execute(
        sql.raw(
          "UPDATE stop SET arrived_at = '2026-05-01T10:00:00.000Z', departed_at = '2026-05-01T10:30:00.000Z' WHERE transport_order_id = '" +
            c.transportOrderId +
            "'",
        ),
      );
  ...
  it('throws TransportOrderNotFoundError for an unknown id', async () => {
    ...
    await withTxIsolation(testDb, async (tx) => {
      ...
      const op = createOperatorContext();
      try {
        await svc.findById('00000000-0000-0000-0000-000000000000', op);
      } catch (e) {
        thrown = e;
      }
    });
  ...
  it('throws TransportOrderNotFoundError when the order is in a different company', async () => {
    ...
    await withTxIsolation(testDb, async (tx) => {
      ...
      try {
        await svc.findById(created.transportOrderId, op2);
      } catch (e) {
        thrown = e;
      }
    });
