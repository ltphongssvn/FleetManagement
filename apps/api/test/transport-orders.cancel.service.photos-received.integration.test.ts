...
async function seedActivePair(tx: TestTx, op: ReturnType<typeof createOperatorContext>): Promise<{ operatorId: string; vehicleId: string }> {
  ...
  const [d] = await tx.insert(driver).values({ ...tn, fullName: 'PhotoGuardDriver', operatorId: op.operatorId }).returning({ driverId: driver.driverId });
  const [v] = await tx.insert(vehicle).values({ ...tn, plate: 'PGX-' + randomUUID().slice(0, 4) }).returning({ vehicleId: vehicle.vehicleId });
...
async function insertReceivedManifest(tx: TestTx, op: ReturnType<typeof createOperatorContext>, transportOrderId: string, state: 'verifying' | 'captured' | 'committed'): Promise<void> {
  const tn = { companyId: op.companyId, businessUnitId: op.businessUnitId, depotId: op.depotId, legalEntityId: op.legalEntityId };
  await tx.insert(manifest).values({ ...tn, transportOrderId, manifestCorrelationId: randomUUID(), state });
}
  ...
  it('throws when a verifying manifest exists (photo uploaded, extraction pending)', async () => {
    ...
    await withTxIsolation(testDb, async (tx) => {
      ...
      const created = await createSvc.create({
        ...
        roadRun: { plannedStartAt: '2026-05-11T07:00:00.000Z', assignedOperatorId: operatorId, assignedAssetId: vehicleId },
      }, op);
  ...
  it('still allows cancel when only a pending manifest row exists (no photo yet)', async () => {
    ...
    await withTxIsolation(testDb, async (tx) => {
      ...
      const created = await createSvc.create({
        ...
        stops: [{ sequence: 1, stopType: 'pickup' }],
        roadRun: { plannedStartAt: '2026-05-12T07:00:00.000Z', assignedOperatorId: operatorId, assignedAssetId: vehicleId },
      }, op);
      const tn = { companyId: op.companyId, businessUnitId: op.businessUnitId, depotId: op.depotId, legalEntityId: op.legalEntityId };
      ...
      const res = await cancelSvc.cancel(created.transportOrderId, { reason: 'customer_request' }, op);
