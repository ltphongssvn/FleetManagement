...
async function seedOrderGraph(co: string, ref: string, toId: string, rrId: string, stopId: string): Promise<void> {
  await testDb.db.execute(sql.raw(
    ...
    q(toId) + ', ' + tenancy(co) + ', ' + q(ref) + ', ' + q('2026-06-11T08:00:00.000Z') + ', now())'));
  await testDb.db.execute(sql.raw(
    ...
    q(rrId) + ', ' + tenancy(co) + ', ' + q('completed') + ', ' + q(co) + ', ' + q(co) + ', ' +
    q('2026-06-11T09:00:00.000Z') + ', ' + q('2026-06-11T14:00:00.000Z') + ', ' + q('2026-06-11T08:05:00.000Z') + ')'));
  await testDb.db.execute(sql.raw(
  ...
  await testDb.db.execute(sql.raw(
    ...
    q(stopId) + ', ' + tenancy(co) + ', ' + q(toId) + ', 1, ' + q('pickup') + ', ' +
    q('2026-06-11T09:30:00.000Z') + ', ' + q('2026-06-11T10:00:00.000Z') + ')'));
}
async function seedManifest(id: string, co: string, toId: string, stopId: string | null, state: string, createdAt: string, committedAt: string | null, reason: string | null): Promise<void> {
  await testDb.db.execute(sql.raw(
    ...
    q(id) + ', ' + tenancy(co) + ', ' + q(toId) + ', ' + q(id) + ', ' + q(state) + ', ' +
    (stopId === null ? 'NULL' : q(stopId)) + ', ' + q(createdAt) + ', ' +
...
describe('@fleet/api - AdminOrderTimelineController (integration)', () => {
  ...
  it('returns the full ordered event stream and validates against the contract', async () => {
    ...
    await seedManifest(M1, OP.companyId, TO, S1, 'committed', '2026-06-11T09:40:00.000Z', '2026-06-11T09:45:00.000Z', null);
    ...
    expect(types).toEqual([
      'order_created', 'run_created', 'run_started', 'stop_arrived',
      'manifest_negotiated', 'manifest_committed', 'stop_departed', 'run_completed',
    ]);
  ...
  it('surfaces legacy unbound manifests with boundStopSequence null and rejected with reasonText', async () => {
    ...
    await seedManifest(M1, OP.companyId, TO, null, 'committed', '2026-06-11T09:40:00.000Z', '2026-06-11T09:45:00.000Z', null);
    await seedManifest(M2, OP.companyId, TO, null, 'rejected', '2026-06-11T09:50:00.000Z', null, 'object_missing');
  ...
  it('emits order_cancelled when the order carries cancellation columns', async () => {
    ...
    await testDb.db.execute(sql.raw(
      ...
      ', cancellation_reason=' + q('customer_request') + ' WHERE transport_order_id=' + q(TO)));
  ...
  it('covers in-flight orders: run not started, stop untouched, verifying + defensively-null committedAt manifests', async () => {
    ...
    await testDb.db.execute(sql.raw(
      'INSERT INTO transport_order (transport_order_id, company_id, business_unit_id, depot_id, legal_entity_id, external_ref, created_at, updated_at) VALUES (' +
      q(TO) + ', ' + tenancy(co) + ', ' + q('XTT.06-010') + ', ' + q('2026-06-11T08:00:00.000Z') + ', now())'));
    await testDb.db.execute(sql.raw(
      ...
      q(RR) + ', ' + tenancy(co) + ', ' + q('planned') + ', ' + q(co) + ', ' + q(co) + ', ' + q('2026-06-11T08:05:00.000Z') + ')'));
    await testDb.db.execute(sql.raw(
      'INSERT INTO road_run_transport_order (road_run_id, transport_order_id, company_id, business_unit_id, depot_id, legal_entity_id, sequence) VALUES (' +
      q(RR) + ', ' + q(TO) + ', ' + tenancy(co) + ', 1)'));
    await testDb.db.execute(sql.raw(
      ...
      q(S1) + ', ' + tenancy(co) + ', ' + q(TO) + ', 1, ' + q('pickup') + ')'));
    ...
    expect(types).toEqual(['order_created', 'run_created', 'manifest_negotiated', 'manifest_negotiated']);
  });
});
