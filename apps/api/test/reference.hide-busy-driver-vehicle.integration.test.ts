  ...
  describe('drivers(op)', () => {
    it('excludes a driver bound to a started road_run while keeping an idle paired driver', async () => {
      const result = await withTxIsolation(testDb, async (tx) => {
        ...
        const [drvIdle] = await tx
          .insert(driver)
          .values({
            ...tenancy(op),
            fullName: 'AAA IdleDrv',
            active: true,
            operatorId: opIdle,
          })
          .returning();
        const [drvBusy] = await tx
          .insert(driver)
          .values({
            ...tenancy(op),
            fullName: 'BBB BusyDrv',
            active: true,
            operatorId: opBusy,
          })
          .returning();
        const [vehIdle] = await tx
          .insert(vehicle)
          .values({
            ...tenancy(op),
            plate: 'IDLE-01',
        ...
        const [rrBusy] = await tx
          .insert(roadRun)
          .values({
            ...tenancy(op),
            state: 'started',
            assignedOperatorId: opBusy,
            assignedAssetId: vehBusy.vehicleId,
            startedAt: new Date(),
          })
          .returning();
        const [orderBusy] = await tx
        ...
        await tx.insert(roadRunTransportOrder).values({
          ...tenancy(op),
          roadRunId: rrBusy.roadRunId,
          transportOrderId: orderBusy.transportOrderId,
          sequence: 1,
        });
        return (await svc.drivers(op)).items;
      });
      expect(result).toEqual([
        { id: '00000000-0000-0000-0000-000000003001', label: 'AAA IdleDrv' },
      ]);
    });
    it('keeps a driver whose only road_run is completed (terminal -> free again)', async () => {
      const result = await withTxIsolation(testDb, async (tx) => {
        ...
        const [drvDone] = await tx
          .insert(driver)
          .values({
            ...tenancy(op),
            fullName: 'AAA DoneDrv',
            active: true,
            operatorId: opDone,
          })
          .returning();
        const [vehDone] = await tx
          .insert(vehicle)
          .values({
            ...tenancy(op),
            plate: 'DONE-01',
        ...
        await tx.insert(driverVehicleAssignment).values({
          ...tenancy(op),
          driverId: drvDone.driverId,
          vehicleId: vehDone.vehicleId,
        });
        await tx.insert(roadRun).values({
          ...tenancy(op),
          state: 'completed',
          assignedOperatorId: opDone,
          assignedAssetId: vehDone.vehicleId,
          startedAt: new Date(),
      ...
      expect(result).toEqual([
        { id: '00000000-0000-0000-0000-000000003010', label: 'AAA DoneDrv' },
      ]);
    });
  });
