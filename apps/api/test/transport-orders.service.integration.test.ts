...
async function seedActivePair(
  ...
  const [d] = await tx
    .insert(driver)
    .values({ ...tn, fullName: 'INT', operatorId })
    .returning({ driverId: driver.driverId });
  const [v] = await tx
    .insert(vehicle)
    .values({ ...tn, plate: 'INT-' + operatorId.slice(0, 4) })
...
describe('@fleet/api - TransportOrdersService (integration)', () => {
  ...
  it('creates transport_order + stops + road_run for a paired driver/truck', async () => {
    await withTxIsolation(testDb, async (tx) => {
      ...
      const stopCount = await tx.execute<{ count: string }>(
        sql.raw('SELECT COUNT(*)::text as count FROM stop'),
      );
  ...
  it('creates road_run and writes 3 append paths', async () => {
    await withTxIsolation(testDb, async (tx) => {
      ...
      const auditSql =
        'SELECT COUNT(*)::text as count FROM fleet_audit_log WHERE event_type = ' +
        qt +
        'road_run.created' +
        qt;
      const feedSql =
        'SELECT COUNT(*)::text as count FROM sync_change_feed WHERE aggregate_type = ' +
        qt +
        'road_run' +
        qt;
      const obSql =
        'SELECT COUNT(*)::text as count FROM outbox WHERE queue_name = ' + qt + 'projections' + qt;
