  ...
  try {
    ...
    if (runFlag !== undefined) {
      ...
      const orderRows = await db
        .select({ r: transportOrder.externalRef })
        .from(transportOrder)
        .where(eq(transportOrder.transportOrderId, firstLink.orderId));
    ...
    const runs = await db.select().from(roadRun).where(inArray(roadRun.roadRunId, runIds));
    ...
    process.stdout.write(
      'INSPECT_RESULT ' + JSON.stringify({ orderState: order.state, runStates }, bigintSafe) + '\n',
    );
  } finally {
    await app.close();
  }
