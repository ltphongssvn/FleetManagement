...
async function main(): Promise<void> {
  ...
  const app = await NestFactory.createApplicationContext(ProjectionRebuildModule, { logger: ["error", "warn"] });
  try {
    ...
    const order = rows[0];
    if (order === undefined) { process.stdout.write("REVIEW_RESULT no-order-for-ref" + nlLit); return; }
    const op = {
