...
export class AdminOrderTimelineService {
  ...
  async getByExternalRef(input: TimelineInput): Promise<OrderTimeline> {
    const [order] = await this.db.select().from(transportOrder)
      .where(and(
    ...
    const runs = await this.db.select({ run: roadRun }).from(roadRunTransportOrder)
    ...
    for (const { run } of runs) {
      events.push({ eventType: 'run_created', at: run.createdAt.toISOString(), roadRunId: run.roadRunId });
      if (run.startedAt !== null) {
        events.push({ eventType: 'run_started', at: run.startedAt.toISOString(), roadRunId: run.roadRunId });
      }
      if (run.completedAt !== null) {
        events.push({ eventType: 'run_completed', at: run.completedAt.toISOString(), roadRunId: run.roadRunId });
      }
    }

    const stops = await this.db.select().from(stop)
      .where(eq(stop.transportOrderId, order.transportOrderId));
    for (const s of stops) {
      if (s.arrivedAt !== null) {
        events.push({ eventType: 'stop_arrived', at: s.arrivedAt.toISOString(), stopSequence: s.sequence, stopType: s.stopType });
      }
      if (s.departedAt !== null) {
        events.push({ eventType: 'stop_departed', at: s.departedAt.toISOString(), stopSequence: s.sequence, stopType: s.stopType });
      }
    }

    const manifests = await this.db.select({ m: manifest, boundSeq: stop.sequence }).from(manifest)
    ...
    for (const { m, boundSeq } of manifests) {
      ...
      events.push({ eventType: 'manifest_negotiated', at: m.createdAt.toISOString(), manifestId: m.manifestId, boundStopSequence });
      if (m.state === 'committed' && m.committedAt !== null) {
        events.push({ eventType: 'manifest_committed', at: m.committedAt.toISOString(), manifestId: m.manifestId, boundStopSequence });
      }
      if (m.state === 'rejected') {
        events.push({
    ...
    events.sort((a, b) => a.at === b.at ? a.eventType.localeCompare(b.eventType) : a.at.localeCompare(b.at));
