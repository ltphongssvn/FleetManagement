...
function parseOneNumber(raw: string): number | null {
  ...
  const m = /^-?(?:[0-9]{1,3}(?:[.,][0-9]{3})+|[0-9]+)(?:[.,][0-9]{1,2})?$/.exec(cleaned);
  if (!m) return null;
  const groups = cleaned.split(/[.,]/);
  const seps = cleaned.replace(/[^.,]/g, '');
  let intPart = groups[0] ?? '';
  let fracPart = '';
  for (let i = 1; i < groups.length; i += 1) {
    const g = groups[i] ?? '';
    const isLast = i === groups.length - 1;
    if (isLast && g.length > 0 && g.length < 3) fracPart = g;
    else if (g.length === 3) intPart += g;
    else return null;
  }
  void seps;
  const n = Number(fracPart === '' ? intPart : intPart + '.' + fracPart);
