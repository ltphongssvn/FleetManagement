...
const ALLOWED_MIME_SET: ReadonlySet<string> = new Set(ALLOWED_MANIFEST_MIME_TYPES as readonly string[]);
...
export type IntakeDecision =
  ...
  | { readonly accepted: false; readonly rejectionCode: IntakeRejectionCode; readonly policyVersion: typeof INTAKE_POLICY_VERSION };
...
export function validateIntake(input: IntakeInput): IntakeDecision {
  ...
  if (!isPositiveFinite(input.expectedSizeBytes) || !isPositiveFinite(input.maxSizeBytes)) {
    return { accepted: false, rejectionCode: 'invalid_input', policyVersion: INTAKE_POLICY_VERSION };
  }
  if (input.actualContentType === null || input.actualSizeBytes === null) {
    return { accepted: false, rejectionCode: 'object_missing', policyVersion: INTAKE_POLICY_VERSION };
  }
  if (!isNonNegativeFinite(input.actualSizeBytes)) {
    return { accepted: false, rejectionCode: 'invalid_input', policyVersion: INTAKE_POLICY_VERSION };
  }
  if (!ALLOWED_MIME_SET.has(input.actualContentType)) {
    return { accepted: false, rejectionCode: 'unsupported_mime_type', policyVersion: INTAKE_POLICY_VERSION };
  }
  if (input.actualContentType !== input.expectedContentType) {
    return { accepted: false, rejectionCode: 'mime_mismatch', policyVersion: INTAKE_POLICY_VERSION };
  }
  if (input.actualSizeBytes > input.maxSizeBytes) {
    return { accepted: false, rejectionCode: 'oversized_file', policyVersion: INTAKE_POLICY_VERSION };
  }
  ...
  if (input.actualSizeBytes > input.expectedSizeBytes + SIZE_ABS_TOLERANCE_BYTES) {
    return { accepted: false, rejectionCode: 'size_mismatch', policyVersion: INTAKE_POLICY_VERSION };
  }
  ...
  if (input.providedHash !== null) {
    if (input.computedHash === null) {
      return { accepted: false, rejectionCode: 'hash_missing', policyVersion: INTAKE_POLICY_VERSION };
    }
    if (input.providedHash !== input.computedHash) {
      return { accepted: false, rejectionCode: 'hash_mismatch', policyVersion: INTAKE_POLICY_VERSION };
    }
  }
  // Fail-closed: virus scan must complete before accept.
  if (input.virusScanClean === null) {
    return { accepted: false, rejectionCode: 'virus_scan_pending', policyVersion: INTAKE_POLICY_VERSION };
  }
  if (!input.virusScanClean) {
    return { accepted: false, rejectionCode: 'virus_detected', policyVersion: INTAKE_POLICY_VERSION };
  }
