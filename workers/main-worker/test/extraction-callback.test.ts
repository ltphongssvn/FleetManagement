...
describe('FetchExtractionCallback', () => {
  it('POSTs the SSOT body to /upload/extraction-result with bearer token', async () => {
    ...
    const cb = new FetchExtractionCallback({
  ...
  it('supports async bearerToken providers', async () => {
    ...
    const cb = new FetchExtractionCallback({
      apiUrl: 'http://api:3000',
      bearerToken: () => Promise.resolve('tok-async'),
      fetchFn,
    });
    await cb.finalize({
      ...RESULT,
  ...
  it('throws on non-2xx so BullMQ retries', async () => {
    const fetchFn = vi
      .fn()
      .mockResolvedValue(new Response('x', { status: 503, statusText: 'Service Unavailable' }));
    const cb = new FetchExtractionCallback({
      apiUrl: 'http://api:3000',
      bearerToken: () => 'tok',
      fetchFn,
  ...
  it('invokes onUnauthorized exactly once on 401, then still throws (BullMQ outer retry)', async () => {
    ...
    const fetchFn = vi
      .fn()
      .mockResolvedValue(new Response('x', { status: 401, statusText: 'Unauthorized' }));
    const cb = new FetchExtractionCallback({
      apiUrl: 'http://api:3000',
      bearerToken: () => 'tok',
      fetchFn,
  ...
  it('does NOT invoke onUnauthorized on other non-2xx statuses', async () => {
    ...
    const fetchFn = vi
      .fn()
      .mockResolvedValue(new Response('x', { status: 503, statusText: 'unavail' }));
    const cb = new FetchExtractionCallback({
      apiUrl: 'http://api:3000',
      bearerToken: () => 'tok',
      fetchFn,
