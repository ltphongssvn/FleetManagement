...
test.describe.serial('Lệnh điều xe board: Tài xế + Xe display driver name and plate', () => {
  test('board row shows assigned driver full name under Tài xế and vehicle plate under Xe', async ({
    page,
    request,
  }) => {
    ...
    await expect(board.getByRole('cell', { name: seed.customerName })).toBeVisible({
      timeout: ROW_VISIBILITY_BUDGET_MS,
    });
    await expect(board.getByRole('columnheader', { name: 'Tài xế' })).toBeVisible({
      timeout: ROW_VISIBILITY_BUDGET_MS,
    });
    ...
    await expect(board.getByRole('cell', { name: seed.driverLabel })).toBeVisible({
      timeout: ROW_VISIBILITY_BUDGET_MS,
    });
    ...
    await expect(board.getByRole('cell', { name: seed.vehicleLabel })).toBeVisible({
      timeout: ROW_VISIBILITY_BUDGET_MS,
    });
  });
});
