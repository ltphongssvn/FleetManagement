...
async function seedAll(api: APIRequestContext): Promise<Seed> {
  ...
  const drv = await adminPost(api, token, '/admin/drivers', { fullName: driverLabel, phone, password: driverLoginValue }, CreateDriverResponseSchema);
  ...
  return {
    ...
    customerName, cargoName, vehicleLabel, pickupName, deliveryName,
  };
}
...
function transportOrderIdOf(externalRef: string): string {
  ...
  return dockerPsql('SELECT transport_order_id FROM transport_order WHERE company_id=' + sq + COMPANY_ID + sq + ' AND external_ref=' + sq + externalRef + sq + ';').stdout.trim();
}
...
async function seedUnrecognisedProof(api: APIRequestContext, token: string, transportOrderId: string): Promise<string> {
  const correlationId = crypto.randomUUID();
  const negotiated = await adminPost(api, token, '/upload/negotiate', {
  ...
  const committed = await adminPost(api, token, '/upload/commit', {
    uploadSessionId: negotiated.uploadSessionId,
    actualSizeBytes: 1024,
  }, CommitUploadResponseSchema);

  await adminPost(api, token, '/upload/intake-result', {
    uploadSessionId: negotiated.uploadSessionId,
    accepted: true,
  }, IntakeResultResponseSchema);

  await adminPost(api, token, '/upload/extraction-result', {
...
function cleanupManifest(manifestId: string): void {
  const sq = String.fromCharCode(39);
  try { dockerPsql('DELETE FROM upload_session WHERE manifest_id=' + sq + manifestId + sq + ';'); } catch { /* tolerate */ }
  try { dockerPsql('DELETE FROM manifest WHERE manifest_id=' + sq + manifestId + sq + ';'); } catch { /* tolerate */ }
}
...
test.describe.serial('Lenh dieu xe: dispatcher manual net-weight entry for an unrecognised phieu-can', () => {
  ...
  test('unrecognised ticket shows Nhap KL, dispatcher enters kg, board reflects it', async ({ page, request }) => {
    ...
    const needsEntry = page.getByTestId('board-stop-netweight-needsentry-' + createdRef + '-pickup-1');
