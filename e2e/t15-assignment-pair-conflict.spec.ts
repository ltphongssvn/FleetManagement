...
test('assigning an already-held vehicle surfaces localized conflict copy, not the English fallback', async ({
  page,
}) => {
  const sq = String.fromCharCode(39);
  cleanup();
  ...
  dockerPsql(
    'INSERT INTO driver_vehicle_assignment (assignment_id, driver_id, vehicle_id, company_id, business_unit_id, depot_id, legal_entity_id) ' +
      'SELECT gen_random_uuid(), d.driver_id, v.vehicle_id, ' +
      sq +
      COMPANY_ID +
      sq +
      ', ' +
      ...
      COMPANY_ID +
      sq +
      ' FROM driver d, vehicle v WHERE d.company_id=' +
      sq +
      COMPANY_ID +
      sq +
      ' AND d.full_name=' +
