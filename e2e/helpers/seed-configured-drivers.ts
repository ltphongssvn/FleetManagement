...
//
// WHY the seed credential is GENERATED, never a literal: a hardcoded constant is
// a credential-shaped string in tracked source. Scanners are value-based, so an
// inline allowlist pragma only silences detect-secrets while GitGuardian still
// flags it in CI, and the pragma rots the moment the line moves. Generating an
// ephemeral per-run value removes the finding at SOURCE: there is no literal to
// match, nothing to suppress, and each run gets a distinct throwaway credential.
//
...
// Ephemeral, generated per run. 24 hex chars satisfies the create-driver
// contract (min 6, max 128) with no literal in tracked source.
const SEED_CREDENTIAL = randomBytes(12).toString('hex');
