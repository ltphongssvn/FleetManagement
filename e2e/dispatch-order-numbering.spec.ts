...
async function createOrderViaApi(
  ...
  if (!res.ok()) {
    throw new Error(
      'POST /transport-orders failed ' + String(res.status()) + ': ' + (await res.text()),
    );
  }
...
function externalRefOf(transportOrderId: string): string {
  ...
  const sql =
    'SELECT external_ref FROM transport_order WHERE transport_order_id=' +
    sq +
    transportOrderId +
    sq +
    ';';
...
test.describe('transport order auto-numbering (T3 invariant) — full layer chain', () => {
  ...
  test.afterEach(async ({ request }) => {
    ...
    while (seededPairs.length > 0) {
      ...
      try {
        ...
        /* already revoked by the test body */
      }
      try {
  ...
  test('L3+L5: API response and DB row both carry server XTT.MM-NNN; client value ignored', async ({
    request,
  }) => {
  ...
  test('L4: two sequential creates produce strictly increasing XT sequence', async ({
    request,
  }) => {
  ...
  test('L4: parallel creates produce two distinct XT numbers (FOR UPDATE atomicity)', async ({
    request,
  }) => {
  ...
  test('L5: order_sequence row seeded (XTT, pad>=3); external_ref unique per company', async ({
    request,
  }) => {
  ...
  test('L6: outbox row for the road_run delta carries the server-assigned XTT.MM-NNN', async ({
    request,
  }) => {
    ...
    const sq = String.fromCharCode(39);
    const sql =
      'SELECT payload::text FROM outbox WHERE payload->>' +
      sq +
      'aggregateType' +
      sq +
      '=' +
  ...
  test('L1+L2: UI submission produces server XTT.MM-NNN and surfaces it on the form', async ({
    page,
    request,
  }) => {
