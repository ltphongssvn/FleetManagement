...
const HANDOFF_PATH =
  process.env['E2E_DRIVER_HANDOFF'] ??
...
async function setupSeed(api: APIRequestContext): Promise<Seed> {
  ...
  const drv = await adminPost(
    ...
    token,
    '/admin/drivers',
    { fullName: driverLabel, phone: driverPhone, password: KNOWN_PASSWORD },
    CreateDriverResponseSchema,
  );
...
function cleanupSeed(seed: Seed): void {
  ...
  try {
    dockerPsql('DELETE FROM road_run WHERE assigned_asset_id=' + v + ';');
  } catch {
    /* tolerate */
  }
  try {
    dockerPsql('DELETE FROM dispatch_board_projection WHERE assigned_asset_id=' + v + ';');
  } catch {
    /* tolerate */
  }
}
...
test.describe.serial('dispatcher creates an order, driver fulfills it (self-seeded)', () => {
  ...
  test.beforeAll(async ({ request }) => {
    seed = await setupSeed(request);
  });
  test.afterAll(() => {
    if (seed) cleanupSeed(seed);
  });
  test('dispatcher UI creates an order bound to the seeded driver, and the driver leg credentials are emitted', async ({
    page,
  }) => {
    ...
    const linked = dockerPsql(
      'SELECT count(*) FROM road_run r ' +
        ...
        'WHERE r.assigned_operator_id=' +
        sq +
        sd.operatorId +
        sq +
        ' AND r.assigned_asset_id=' +
        sq +
        sd.vehicleId +
        sq +
        ';',
  ...
  test('the seeded driver can authenticate and sees their assigned vehicle (in-process, before teardown)', async ({
    request,
  }) => {
    const sd = requireSeed();
