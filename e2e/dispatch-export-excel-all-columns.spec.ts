...
// xe Excel workbook is a DATA export: after the 7 identifying columns (Số lệnh,
// Trạng thái, Khách hàng, Tài xế, Xe, Ngày dự kiến, Số điểm) plus the
// Chênh lệch (pickup-vs-delivery weight diff) column
//   Số lệnh | Trạng thái | Khách hàng | Tài xế | Xe | Ngày dự kiến | Số điểm | Chênh lệch
...
// slot 1, giving 18 columns total:
...
test.describe.serial('export Excel contains all on-screen Lệnh điều xe columns', () => {
  ...
  test('exported workbook header row equals the 18 SSOT columns (Số lệnh + Trạng thái + 5 more identifying + Chênh lệch + per-slot name/kg pairs) in order', async ({ page }) => {
