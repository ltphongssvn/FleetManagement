...
| 10 | Write-side gap: admin-drivers-update.service.ts:56,76 (driver soft-delete cascade) + reference.service.deleteVehicle set order state=cancelled via RAW UPDATE (cancelled_by NULL, machine reason driver_soft_deleted outside CANCEL_REASONS), skipping cascade+audit+appendTriWrite. Route through TransportOrdersCancelService.cancel or replicate the cascade. Sentinel: repair:ghost-runs dry-run stays found=0 | T9 | apps/api/src/admin/admin-drivers-update.service.ts, apps/api/src/reference/reference.service.ts, apps/api/test | queued |
| 11 | sync-protocol dispatch-board-pagination-contract.ts hand-writes ROAD_RUN_TERMINAL_STATES; check dependency direction before deriving from @fleet/domain ROAD_RUN_NON_TERMINAL_STATES (avoid a domain->sync-protocol cycle) | T9 | packages/sync-protocol/src/dispatch-board-pagination-contract.ts, packages/domain | queued |
