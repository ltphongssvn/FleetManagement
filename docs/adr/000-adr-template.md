<!--
File:    FleetManagement/docs/adr/000-adr-template.md
Purpose: Template for new Architecture Decision Records in FleetManagement.
Why:     PDF mandates docs/adr/ for versioned architectural decisions.
         ADRs capture WHY, not HOW — rationale, trade-offs, alternatives,
         and deferred work, so config files can stay focused on config.
-->

# ADR-NNN: <Title>

- **Status**: Proposed | Accepted | Deprecated | Superseded by ADR-XXX
- **Date**: YYYY-MM-DD
- **Deciders**: <names / roles>
- **Related**: ADR-XXX, `path/to/affected/file`

## Context

<What problem forced this decision? What constraints apply? Cite the PDF
section or empirical finding that motivated it.>

## Decision

<The choice made, stated plainly.>

## Rationale

<Why this choice over alternatives. Include empirical evidence (CLI
verification output, doc citations) where applicable.>

## Alternatives Considered

- **Option A**: <what it was, why rejected>
- **Option B**: <what it was, why rejected>

## Consequences

**Positive**: <what this unlocks>
**Negative**: <what this costs>
**Neutral**: <side effects>

## Future Work

<Deferred items that depend on this decision. Each item should name the
trigger condition (e.g., "when apps/* packages land") and the concrete
change required.>

## Verification

<How to empirically confirm the decision still holds. CLI commands,
tests, or metrics.>
